...
- **인수인계 확인**: 인수인계 메모와 서버에서 생성된 수색 이력 요약을 근무 교대/OP 전환 화면에서 조회한다. 요약 생성은 근무 교대 종료/OP 전환 commit 이후 서버가 처리하며, 앱은 요약 생성·재시도 명령을 보내지 않는다.
...
- OpenAI 호출 실패, timeout, schema validation 실패, 금지 문구 검출 시 `search_history_summary.generation_status = FAILED`로 남기고 `content`를 저장하지 않는다. 조회/API/UI는 `summary_unavailable` 상태를 표시하고, 재생성은 서버 관리 재시도/재큐잉 정책으로 처리한다.
- 앱과 웹은 생성된 `search_history_summary`를 조회할 수 있다. 앱과 웹 모두 `READY`/`FAILED`/`GENERATING` 상태와 안전한 요약만 표시하고, client-side 생성/재시도 CTA를 두지 않는다.
