...
- 1분 시험에서는 765 RPS가 통과했지만 같은 요청률을 5분 유지하자 실패했습니다. 지속 시간에 따라 결과가 달라졌으므로 1분 시험값은 서버 처리 한계로 판단하지 않았습니다.
- 5분 시험에서 453 RPS 실행은 HTTP 실패 없이 통과했고, 454 RPS 실행은 2회 모두 HTTP 실패로 중단됐습니다. 최종 확인 3회(453 RPS 1회, 454 RPS 2회)에서 `dropped_iterations`는 모두 0건이었습니다.
...
- **문제:** 새 좌표 6개를 추가할 때 누적 좌표·구간을 다시 처리하고 전체 경로 LineString을 다시 저장해, `31 RPS` 시험에서 8시간 경로가 두 번 모두 약 13초 만에 시간 초과됐습니다.
- **변경:** 1차로 요청에서 새로 생긴 좌표·구간·제외 좌표만 저장하고 새 구간·제외 좌표는 각각 한 번의 SQL로 묶었습니다. 2차로 전체 경로 LineString 생성을 좌표 추가 시점에서 조회 시점으로 옮겼습니다.
- **결과:** 업무폰 468대의 주기적 전송과 혼합 복구를 누락 없이 마쳤고, 5분 서버 처리 한계는 `453 RPS 통과 / 454 RPS 실패`에서 `719 RPS 통과 / 720 RPS 실패`로 바뀌었습니다.
- **트레이드오프:** 좌표 추가 비용은 누적 경로 길이와 분리됐지만, 여러 사용자가 전체 경로를 동시에 조회할 때의 LineString 생성 비용은 아직 측정하지 않았습니다.

원인 분석과 30분·8시간·24시간 경로 검증은 [Issue #16](https://github.com/sonic8-8/suri-map/issues/16)에 기록했습니다.

## 검증 결과

### 재측정 결과: 복구 1분 5초와 719 RPS 통과

2026년 9월 4일, 같은 Hetzner 개발 환경에서 경로 저장 최적화가 반영된 서버를 대상으로 세 시나리오를 다시 실행했습니다.

| 시나리오 | 최적화 전 | 최적화 후 |
| --- | --- | --- |
| 업무폰 468대의 주기적 전송 | 별도 유효 결과 없음 | 5분간 9,380건 완료, HTTP p95 `27.8ms` |
| 30분치 재전송 중 새 요청 유입 | 재전송 완료 최대 `4분 14초`, HTTP p95 `3.92초`, 새 요청 시작 지연 p95 `3분 32초` | 재전송 완료 최대 `1분 5초`, HTTP p95 `818.29ms`, 새 요청 시작 지연 p95 `36.27초` |
| 5분 서버 처리 한계 | `453 RPS` 통과, `454 RPS` 반복 실패 | `719 RPS` 통과, `720 RPS` 실패 |

성공한 세 실행은 각각 `요청 9,380건·좌표 56,280개`, `요청 65,520건·좌표 393,120개`, `요청 215,701건·좌표 1,294,206개`가 PostgreSQL 기록과 일치했습니다. HTTP 실패와 제외·중복 좌표는 0건이었고, 완료된 멱등성 처리 기록 수도 요청 수와 같았습니다.

720 RPS 실행은 약 23초 뒤 HTTP 실패 3건으로 중단됐습니다. 719 RPS와 720 RPS 모두 `dropped_iterations`는 0건이었습니다. 현재 조건의 마지막 통과값은 453 RPS에서 719 RPS로 266 RPS, 약 58.7% 증가했습니다.
...
주기적 전송에서는 데이터베이스 커넥션 풀을 기다린 요청이 없었습니다. 혼합 복구와 719 RPS 시험에서는 커넥션 10개를 모두 사용했고, 대기 요청은 최대 189개였습니다. 719 RPS 실행의 App 서버 CPU는 최대 71.5%, Ops 부하 발생기 CPU는 최대 57.3%였으므로 부하 발생기가 먼저 포화된 결과로 보지는 않았습니다.

높은 부하의 데이터베이스 커넥션 대기는 남아 있지만 이번 결과만으로 특정 SQL이나 커넥션 풀 크기를 추가 병목으로 단정하지 않습니다. `719 RPS 통과 / 720 RPS 실패`도 현재 서버 사양, 요청 형태와 5분 조건의 결과이지 운영 안전 처리량이나 서비스 수준 협약(SLA)은 아닙니다.

24시간 경로의 좌표 34,560개를 LineString으로 조립한 시간은 중앙값 약 `54ms`였습니다. 이번 재측정은 이미 충족한 업무폰 468대의 복구 조건을 최적화 후 구현에서 다시 확인한 것이므로 해결 기준을 새로 체크하지 않습니다. 다음에는 상황판의 동시 조회 부하와 지도 표시 시간을 측정하며, Issue #8은 최종 GPS 수집·전송 기준과 현장 허용 기준을 정할 때까지 열어 둡니다.
