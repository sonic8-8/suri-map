...
- 사건은 사용자가 직접 만들지 않는다. MVP/시연은 mock 112 배정 사건을 가져온다.
- 계정은 팀 계정, 순찰차 계정, 지휘 계정 기준이다.
- GPS 경로의 기록 주체는 팀 업무폰 또는 순찰차 업무폰 PolicePhone다.
- 지도 범위는 `search_area.area_level = OVERALL`로 관리한다. 특정 지형 전용 범위 개념을 만들지 않는다.
...
| Web 상황판 | 지휘·상황 공유, 구역·OP 관리, 인수인계 | 배정 사건 가져오기/종료, 지도 기준 범위, 구역 분할·완료, OP, 인수인계 메모, 수색 이력 요약 |
| System | mock 112 import, 112/mock assignment polling/import, 동기화, 이벤트, 파기 | 사건·지원부대 배정 결과 반영, OP1 자동 생성, 이벤트 fanout, purge orchestration |
...
| Spec | 이름 | 핵심 책임 | Lane |
|---|---|---|---:|
| S1-1 | Incident Import & Assignment | mock 112 사건 가져오기, 사건 lifecycle, 실종자 도메인 데이터, 사건 배정 결과 | L1 |
...
| S2 | Search Area & Assignment | 전체/부대/팀 수색 구역, 구역 배정, 구역 상태 이력 | L3 |
| S3-1 | PolicePhone Path Collection | PolicePhone 경로, 차량·도보 구간 | L4 |
| S3-2 | Situation Board Shell & API Assembly | 상황판 shell, 원본 API 조립, slot merge/rendering, OP 비교 화면 | L6 |
| S4 | Realtime Event Hub | SSE refetch signal, event envelope, `EventHub.publish`, `EventFanout`, `event_dispatch_job`, `sse_replay_event`, `event_dispatch_target` | L2 |
| S5 | Markers / Photo / Marker Notification | 현장 마커, 사진, marker notification payload/recipient 계산, `FcmDispatcher` adapter | L5 |
| S6 | Offline Sync & Local Warnings | Outbox, idempotency, sync status, 로컬 경고 | L4 |
| S7 | Offline Tiles & Incident Package | tileserver, 오프라인 패키지 manifest/installation | L6 |
| S8 | Operational Period, Duty Shift & Handover | OP, 근무 구간, 인수인계 메모, 수색 이력 요약 | L3 |

## 1.1 Lane별 작업 경계

| Lane | 담당 |
|---|---|
| L1 | 사건 가져오기, 사건 종료, 실종자 도메인 데이터, 사건 배정 결과 |
...
| L6 | 상황판 shell, API 조립, offline package, tileserver |

S4는 공용 계약이지만 실구현·유지보수·버그 수정은 L2가 주도한다. `event_dispatch_job`, `sse_replay_event`, `event_dispatch_target` row와 fanout orchestration은 S4가 단일 소유한다. Domain Spec(S1-1/S2/S3-1/S5/S8 등)은 이벤트 전파 row를 owns하지 않고 해당 이벤트 payload와 발행 요청 생성·검증만 책임진다. 이벤트 payload 변경은 관련 Spec 오너와 S3-2, S4 LGTM이 필요하다.
...
|---|---|---|---|
| L1 | S1-1 사건 import/lifecycle/assignment | SC-01 import, 사건 목록/상세, 사건 종료 요청 fixture | S1-2 auth fixture, S4 event envelope |
| L2 | S1-2/S1-3/S4 계정·PolicePhone·권한·운영 기록·이벤트 허브 | team/police-phone auth harness, SSE/replay harness, purge orchestration harness | seed account/policePhone ID 목록, event payload owner LGTM |
...
| L4 | S3-1/S6 경로·Outbox·로컬 경고·복구 동기화 | SC-05/SC-07/SC-09 path/outbox harness | S1-2 policePhone fixture, S2 overall area query, S4 event envelope |
...
| L6 | S3-2/S7 상황판 shell/API 조립·offline package | board slot merge harness, SC-03 package harness, API/SSE refetch convergence harness | §9.2 slot registry, S4 refetch signal, S2/S5/S8 query contracts |
...
3. 공용 계약은 1주차에 먼저 동결한다: S1-2 account/policePhone fixture, S4 event envelope/refetch signal, S3-2 board slot registry, S6 idempotency/outbox shape, S2 geometry fixture.
...
| 후보 | 판단 | 이유 | 향후 분리 조건 |
|---|---|---|---|
| S1-1 Incident Import & Assignment | 유지 | 사건 lifecycle, import, incident_assignment는 사건 접근권한과 `OPEN/CLOSED` guard를 같이 만든다. | 실제 112 연동·조직 편제 동기화가 별도 서비스로 커지면 `Incident Lifecycle`과 `Incident Assignment Import` 분리 |
| S3-2 Situation Board Shell & API Assembly | 유지 | shell, slot mount, API 조립, refetch 수렴은 최종 통합 owner가 하나여야 merge 충돌을 줄인다. | 상황판 API 조립과 Web shell이 서로 다른 배포 단위가 되면 `Board Shell`과 `Board API Assembly` 분리 |
...
| S8 Operational Period, Duty Shift & Handover | 유지 | OP, duty shift, handover memo, search history summary는 OP 이력이라는 같은 도메인 입력을 공유한다. | 수색 이력 요약이 별도 모델 운영·프롬프트 버전·비동기 job product로 커지면 `Operational Period/Handover`와 `Search History Summary` 분리 |
...
2. 사건 import는 성공 시 `OPEN` incident와 OP1을 같은 트랜잭션에서 생성하고, 실패 시 incident row를 남기지 않는다.
...
7. 이벤트 발행 요청은 domain transaction 안에서 S4 `event_dispatch_job`로 stage하고, commit 이후 fanout은 S4 `EventFanout`이 수행한다.
...
- `missing_person`
- `incident_assignment`
- mock 112 incident import adapter and 112/mock assignment polling/import adapter
...
- `POST /incidents/import` imports mock 112 assigned incident data and creates `incident`, `missing_person`, `incident_assignment`, and OP1 in one transaction. Import failure rolls back all rows and event staging.
- Incident lifecycle stores only `OPEN` and `CLOSED`; writes after close fail with `incident_closed`.
- Suri-Map does not expose public assignment or commander mutation APIs. Additional support assignments are reflected only through 112/mock·seed polling/import into `incident_assignment` and emit `INCIDENT_ASSIGNMENT_CHANGED`.
...
- import 성공 시 `OPEN` 사건과 OP1이 함께 생성된다.
- OP1 생성 실패 시 incident row를 남기지 않고 import 전체를 rollback한다.
- 사건 종료는 terminal이다. 재오픈 API 없음.
- 종료 시 실종자 도메인 데이터는 active DB와 API 응답에서 제거한다.
...
- `SecurityContext.organizationType`
...
- App-only PolicePhone APIs reject missing, unregistered, or unassigned PolicePhone contexts with the matching guard error code.
- `POST /police-phones/{policePhoneId}/heartbeat` updates the single freshness source and emits `POLICE_PHONE_HEARTBEAT_UPDATED`.
- `POST /fcm/tokens` stores tokens for authenticated app PolicePhone contexts without granting notification routing ownership.
...
- 동일 채널 중복 로그인 정책은 운영 전 확정 전까지 `same account multi-session allowed, PolicePhone heartbeat wins`로 둔다.
...
- `location_data_access_audit` (internal/system-only)
- `incident_data_purge` (internal/system-only)
...
- `incident_data_purge.schema.json`
- `location_data_access_audit.schema.json`
...
- `INCIDENT_PURGED` is published only after every purge hook succeeds; failed hooks leave retryable `incident_data_purge` state.
...
- `RetentionPurgeStatus`, `incident_data_purge`, `location_data_access_audit`, 운영 로그 sink는 internal/system-only 계약이다. 사용자 UI, 앱 캐시, 오프라인 패키지, S3-2 상황판 조회에 노출하지 않는다.
...
- purge hook은 idempotent해야 한다. 실패 시 `incident_data_purge`을 failed/retryable로 남기고 `INCIDENT_PURGED`는 모든 hook 성공 후에만 발행한다.

---

### S2 · Search Area & Assignment
...
- `events/search_area_assignment.payload.schema.json` for `SEARCH_AREA_ASSIGNMENT_CHANGED`
...
- S1-1: incident lifecycle, assignment
...
- Overall search area APIs enforce one active `area_level = OVERALL` area per incident, valid Polygon geometry, and Web command authorization.
- Search area create/split/state APIs persist `search_area_history` with account, time, OP, previous state, next state, and memo.
- Search area and assignment writes publish the matching `PublishRequest.*` contract with the declared event payload schema.
...
- 지도 기준 범위는 `search_area.area_level = OVERALL`인 사건별 1개 active row로 표현한다.
- `search_area`는 유효한 Polygon만 허용한다. 빈 geometry, 자기 교차, SRID 불일치는 `invalid_geometry`로 거부한다.
...
- `events/search_path.payload.schema.json` for `SEARCH_PATH_STARTED`, `SEARCH_PATH_ENDED`
- `events/search_path.payload.schema.json` for `PATH_APPENDED`
- `events/search_path_segment.payload.schema.json` for `SEARCH_PATH_SEGMENT_UPDATED`
...
### S3-2 · Situation Board Shell & API Assembly
...
- shared state wiring
- board API assembly from source queries
...
- S1-1: incident, missing person, assignment
- S1-2: account, PolicePhone heartbeat
- S2: overall search area, search area
- S3-1: path, segment
- S4: SSE refetch signal
- S5: marker, marker notification events
- S7: offline package installation, tile server health
- S8: OP, duty shift, handover memo, search history summary
...
- `GET /incidents/{incidentId}/board` returns `BoardDTO` by assembling source table query contracts within the board API performance target.
...
- SSE updates are refetch signals only. The shell deduplicates by `eventId` and reloads affected source APIs or the board API; no separate board read-model DB is written.
...
- S4 `EventFanout`은 SSE refetch signal만 보낸다. S3-2는 signal 이후 API 재조회와 화면 렌더링만 소유한다.
- S3-2는 별도 board read-model DB 엔티티를 만들지 않는다.
- `lastHeartbeatAt`/`lastSyncAt` 경과는 위치 점 색·외곽선·라벨로 표시한다. 별도 알림은 만들지 않는다.
- 운용 중 PolicePhone 경로는 현재 `SearchPath`의 `policePhoneId` 기준으로 강조한다.
- 초기 뷰포트 대체 순서: overall search area → 최근 활동 위치 → 기본 지역.
...
- `event_dispatch_target.schema.json`

**consumes**

- S1-2: authenticated account and incident access
- S3-2: SSE refetch consumer
...
- `event_dispatch_job` 적재 후 fanout orchestration은 S4 `EventFanout` 경계다. SSE refetch signal 송신, FCM delivery 호출, target별 재시도, 실패 주입 지점은 S4가 정의한다.
- `eventId`는 사건 내 중복 수신 제거의 기준이다.
- SSE replay는 event log 순서를 보존해야 하며, 소비자는 이미 적용한 `eventId`를 다시 적용하지 않는다.
- 인증 실패 시 SSE stream을 종료한다.
- Last-Event-ID가 너무 오래됐으면 `409 gone_refetch_required`를 반환하고 S3-2 board API 재조회로 복구한다.
- `INCIDENT_PURGED` 수신 후 해당 사건의 SSE replay data를 파기한다.
- 실패 주입 지점은 event dispatch job 적재 후 SSE 미송신, `FcmDispatcher` adapter 실패, target dispatch 실패/지연을 포함한다.

---

### S5 · Markers / Photo / Marker Notification
...
- `events/marker_notification.payload.schema.json` for `SUPPORT_REQUEST_CREATED`, `PERSON_FOUND`
...
- `marker_notification`는 알림 recipient/payload 스냅샷 엔티티이며 전파 상태는 기록하지 않는다. 실제 SSE/FCM 상태는 S4 `event_dispatch_target`이 기록한다.
- 지원 요청 알림은 실종팀 지휘 계정과 현장 지휘관 역할 계정 우선이다.
- 실종자 발견 알림은 사건 배정 계정·단말 전체 대상이다.
- 지원 부대 배정 알림은 `INCIDENT_ASSIGNMENT_CHANGED` fanout 시 신규 배정 계정이 운용 중인 활성 `policePhoneId`의 FCM token에만 data message로 전달한다. 이 알림은 `marker_notification` row를 만들지 않으며, 지휘 계정 policePhoneId는 Android FCM recipient로 고정하지 않는다.
...
- S7: package installation status
...
- overall search area 변경 후 manifest revision이 증가해야 한다.
...
### S8 · Operational Period, Duty Shift & Handover
...
- `DutyShiftQuery.byIncident(incidentId)`
- `DutyShiftQuery.current(incidentId, policePhoneId)`
...
- `events/duty_shift.payload.schema.json` for `DUTY_SHIFT_CHANGED`
...
- `PublishRequest.DUTY_SHIFT_CHANGED`
...
- S1-1: incident lifecycle and assignment
- S1-2: role and channel
- S2: search area and assignment
...
- OP1 is created during incident import; OP2 and later require Web command authorization and an allowed creation reason.
- Duty shift writes persist `duty_shift`, update `DutyShiftQuery`, and emit `DUTY_SHIFT_CHANGED`.
- Handover memo writes support app and Web channels, preserve context, and emit `HANDOVER_MEMO_CREATED`.
- Search history summary generation uses only OP/dutyShift/path/marker/memo history and leaves manual memo and OP comparison usable on failure.
...
- OP1은 사건 import 중 자동 생성된다.
- OP2 이후는 수동 생성하며, 생성 권한·채널은 `spec/boundaries.md §4.6 Channel / Role Matrix` `OP 생성` 행과 S1-2 `@RequireChannel`, `@RequireRole` 기준 적용.
- OP 생성 사유는 `INITIAL`, `RE_SEARCH`, `AREA_CHANGED`, `OTHER` 중 하나다. `INITIAL`은 OP1 자동 생성에만 사용하고, 근무 교대는 OP 사유가 아니라 `duty_shift`로 기록한다.
- `OTHER` 사유는 사유 메모 또는 인수인계 메모를 함께 남긴다.
- 수색 이력 요약은 OP/근무 구간/경로/마커/메모 기반 이력 요약만 한다.
...
| `police_phone_required` | PolicePhone가 필요한 앱 요청에 PolicePhone 없음 | 400 |
| `police_phone_not_registered` | 등록되지 않았거나 세션과 연결되지 않은 PolicePhone | 403 |
| `police_phone_not_assigned` | PolicePhone가 사건·팀·OP 배정에 연결되지 않음 | 403 |
...
공간 geometry를 받는 domain write는 공통으로 빈 geometry, SRID 불일치, geometry type 불일치를 `invalid_geometry`로 거부하되, 각 geometry 필드의 도메인 소유권은 해당 Spec의 owns 기준을 따른다. S2는 `search_area` Polygon 도메인 검증과 `SearchAreaQuery.overallOf(incidentId)` 제공을 소유한다. S3-1은 `search_path`/`search_path_segment`, S5는 `marker`/`photo` 위치 필드 검증을 이 공통 규칙과 S2의 overall search area query를 입력으로 수행한다.
...
6. S4 `EventHub.publish(PublishRequest)` 호출과 `event_dispatch_job` stage
...
| `DUTY_SHIFT_CHANGED` | S8 | S3-1, S3-2 |
| `HANDOVER_MEMO_CREATED` | S8 | S3-2 |
| `SEARCH_HISTORY_SUMMARY_CHANGED` | S8 | S3-2 |

Event payload는 REST response DTO, S6 `write_operation.schema.json`, S4 event_dispatch_job/SSE, S3-2 `board.schema.json`, S5 FCM payload가 같은 필드명을 공유한다. Payload root의 공통 비교 필드는 `id`, `status`, `version`이며, 적용 가능한 이벤트는 `opId`, `policePhoneId`도 같은 이름으로 포함한다. SSE envelope는 §9.1 기준 적용. FCM은 동일 payload subset에 §9.1 envelope 중 알림 전달에 필요한 메타만 얹는다.
...
| `POLICE_PHONE_HEARTBEAT_UPDATED` | `events/police_phone_heartbeat.payload.schema.json` | 1 | `id`, `status`, `policePhoneId` |
| `SEARCH_AREA_CHANGED` | `events/search_area.payload.schema.json` | 1 | `id`, `status`, `version` |
| `SEARCH_AREA_ASSIGNMENT_CHANGED` | `events/search_area_assignment.payload.schema.json` | 1 | `id`, `status`, `version`, `opId` |
| `SEARCH_PATH_STARTED` | `events/search_path.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
| `SEARCH_PATH_ENDED` | `events/search_path.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
| `PATH_APPENDED` | `events/search_path.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
| `SEARCH_PATH_SEGMENT_UPDATED` | `events/search_path_segment.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
...
| `SUPPORT_REQUEST_CREATED` | `events/marker_notification.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
| `PERSON_FOUND` | `events/marker_notification.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
| `OFFLINE_PACKAGE_INSTALLATION_CHANGED` | `events/offline_package_installation.payload.schema.json` | 1 | `id`, `status`, `version`, `policePhoneId`, `manifestVersion` |
| `OP_TRANSITIONED` | `events/operational_period.payload.schema.json` | 1 | `id`, `status`, `version`, `opId` |
| `DUTY_SHIFT_CHANGED` | `events/duty_shift.payload.schema.json` | 1 | `id`, `status`, `version`, `opId`, `policePhoneId` |
...
2. `missing_person`과 실종자 `photo_object_key` pointer를 active DB/API 응답에서 제거
...
| `police_phone` | S1-2 |
| `fcm_token` | S1-2 |
| `location_data_access_audit` | S1-3 |
| `incident_data_purge` | S1-3 |
| `search_area` | S2 |
| `search_area_assignment` | S2 |
| `search_area_history` | S2 |
| `search_path` | S3-1 |
| `search_path_segment` | S3-1 |
| `event_dispatch_job` | S4 |
| `sse_replay_event` | S4 |
| `event_dispatch_target` | S4 |
| `marker` | S5 |
| `photo` | S5 |
| `marker_notification` | S5 |
| `idempotency_record` | S6 |
...
|---|---|---|
| FR-01 사건 관리 | S1-1 | mock 112 import, OP1 자동 생성 |
...
| FR-10 단서 마커 | S5 | 앱 생성 |
| FR-11 OP별 레이어 선택·비교 | S8/S3-2 | board consumes OP path/area assignment/marker/memo |
| FR-12 담당 구역 배정 | S2/S8 | search_area_assignment + current OP |
| FR-13 구역 상태·수색 이력 | S2/S8 | search_area_history 기반 historyCount |
...
| FR-27 초기 뷰포트 | S3-2 | overall search area 대체 순서 |
| FR-28 미전송 큐 | S6 | Android dashboard |
| FR-29 로컬 경고 | S6 | S6가 S7 package installation 상태를 입력으로 소비해 local warning을 표시한다. S7은 package manifest/installation owner이고 local warning monitor owner가 아니다. |
| FR-30 바텀시트 마킹 | S5 | Android |
| FR-31 사건 오프라인 패키지 | S7 | manifest/installation, overall search area 기반 tile/revision/stale |
| FR-32 OP 히스토리 레이어 | S8 | OP별 경로·마커·완료 구역·인수인계 메모 |
| FR-33 차량·도보 구간 | S3-1 | search_path_segment |
| FR-34 수색 경로 | S3-1/S8 | current OP + duty shift |
| FR-35 순찰차 경로 레이어 | S3-1/S3-2 | `account_type=PATROL_CAR`, `movement_type=VEHICLE` |
| FR-37 인수인계 메모 | S8/S3-2 | OP·근무 구간·구역·경로 context |
| FR-39 수색 이력 요약 | S8/S3-2 | 자동 판단 금지 |
...
- `write-common`: `@RequireOpenIncident`, `@IdempotentWrite` -> `incident_closed`, `idempotency_mismatch`, `write_conflict`
...
| `POST /incidents/import` | S1-1 | 웹 지휘 계정 | HTTPS | `web-command` | - |
...
| `POST /markers/{markerId}/photos/upload-url` | S5 | 앱 | HTTPS | `app-police-phone`, `incident-read`, `write-common` | - |
| `POST /markers/{markerId}/photos/{photoId}/attach` | S5 | 앱 | HTTPS | `app-police-phone`, `incident-read`, `write-common` | `internal-caller`: outbox replay |
...
| `POST /operational-periods` | S8 | 웹 | HTTPS | `web-command`, `incident-read`, `write-common` | `internal-caller`: OP1 자동 생성 |
| `GET /incidents/{incidentId}/operational-periods` | S8 | 앱, 웹, S3-2, S7 | HTTPS | `public-session`, `incident-read` | - |
| `POST /duty-shifts` | S8 | 앱, 웹 | HTTPS | `field-or-web-write`, `incident-read`, `write-common`, `@RequireCurrentOp` | - |
| `PATCH /duty-shifts/{dutyShiftId}` | S8 | 앱, 웹 | HTTPS | `field-or-web-write`, `incident-read`, `write-common`, `@RequireCurrentOp` | - |
...
| `@RequireOpenIncident` | S1-1 | `incident_closed` | 사건 OPEN 상태 확인 |
...
| `@PublishEvent` | S4 | - | event_dispatch_job publish hook |
...
| `area` | S2 | S3-2 | `SearchAreaQuery.byIncident`, `SearchAreaQuery.byOp` | 구역 폴리곤·상태 표시 |
...
| `op_history` | S8 | S3-2 | `OperationalPeriodQuery.list`, `OP_TRANSITIONED`, `DUTY_SHIFT_CHANGED` | OP 전환·근무 구간 이력 표시 |
...
| `search_history_summary` | S8 | S3-2 | `SearchHistorySummaryQuery.byOp` | 수색 이력 요약 |
