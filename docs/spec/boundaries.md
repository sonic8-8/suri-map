...
- `POST /api/internal/mock-112/events` is the default mock-112 ingestion path. `INCIDENT_READY` creates `incident`, `missing_person`, `incident_assignment`, OP1, and emits `INCIDENT_CREATED` only after the domain write is committed. `INCIDENT_CLOSED` closes the already-imported OPEN incident through the same terminal lifecycle as `POST /api/incidents/{incidentId}/close`. `POST /api/incidents/import` remains a WEB command fallback for manual replay/ops support.
...
공간 geometry를 받는 domain write는 공통으로 빈 geometry, SRID 불일치, geometry type 불일치를 `invalid_geometry`로 거부하되, 각 geometry 필드의 도메인 소유권은 해당 Spec의 owns 기준을 따른다. S2는 `search_area.area_level=OVERALL`과 search area Polygon 도메인 검증, `SearchAreaQuery.overallOf(incidentId)` 제공을 소유한다. S3-1은 `search_path`/`search_path_segment` 위치 필드 검증을 이 공통 규칙과 S2의 overall search area query를 입력으로 수행한다. S5 marker 위치 검증은 Point type, 좌표 개수, SRID, NaN, lon/lat 범위 오류처럼 좌표 자체의 유효성만 `invalid_geometry`로 거부한다. 전체 수색구역과 담당 구역은 현장 마커 작성의 선행조건이 아니며, 구역 밖 단서·발견·지원 요청도 저장 가능한 현장 기록으로 취급한다. 담당 TEAM search_area 이탈 안내는 Android 로컬 경계 안내와 `POST /api/search-area-boundary-alerts` 정책으로 별도 처리한다.
...
| SC-12 사건 종료·도메인 데이터 파기 | S1-1, S1-2, S1-3, S6, S7, S4, S3-1, S3-2, S5 | §7 `POST /api/incidents/{incidentId}/close`, `POST /api/internal/mock-112/events` `INCIDENT_CLOSED`, `GET /api/incidents/{incidentId}/events` web unsubscribe/replay stop, §4.4 `INCIDENT_CLOSED`, `INCIDENT_PURGED`, local package purge, app local close cleanup | §9.2 `incident_terminal`, `package_badge` | S1-1 close 또는 mock-112 `INCIDENT_CLOSED` webhook -> S1-3 purge orchestration -> S6/S7 purge hooks -> sanitized terminal/tombstone status -> S3-2 `incident_terminal`; S4 carries `INCIDENT_CLOSED`/`INCIDENT_PURGED` fanout, while S6/S7 handle local/package removal and app local close cleanup |
