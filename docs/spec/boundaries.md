...
- `NotificationRecipientResolver` and `NotificationPayloadFactory` produce delivery rows and FCM payload input without owning fanout orchestration.
...
- OP 생성 사유는 `SHIFT_CHANGE`, `RESEARCH`, `NEW_AREA`, `OTHER` 중 하나다.
