...
- **then**:
  ...
  4. 사진 첨부 시 S3-compatible presigned upload URL 발급, object storage 업로드, attach 흐름으로 저장된다.
...
- **e2e_red_test**:
  ...
  - "사진 presigned upload URL 발급, upload, attach 단계 실패 시 앱은 실패 단계와 사유를 표시하고 업로드 재시도 CTA를 제공한다"
  ...
  - "미등록 PolicePhone이 `POST /markers`, 사진 presigned upload URL 발급, 사진 attach를 호출하면 `403 police_phone_not_registered`를 응답하고 마커·사진이 생성되지 않는다"
  - "등록됐지만 해당 사건/OP에 배정되지 않은 PolicePhone이 `POST /markers`, 사진 presigned upload URL 발급, 사진 attach를 호출하면 `403 police_phone_not_assigned`를 응답하고 앱은 pending 마커를 서버 반영 완료로 바꾸지 않는다"
...
- **board_merge**: `marker` slot + `path` slot + `police_phone_freshness` slot 이벤트 재수신
