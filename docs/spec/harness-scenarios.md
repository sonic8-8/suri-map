...
- **e2e_red_test**:
  ...
  - "`search_path` 포인트가 현재 `overall_search_area` 또는 담당 TEAM `search_area` 밖이어도 좌표 자체가 유효한 EPSG:4326이면 batch를 저장한다. 담당 TEAM 구역 밖 표시는 경로 저장 차단이나 GPS 품질 실패가 아니라 `SEARCH_AREA_BOUNDARY_EXITED` 운영 참고 이벤트로만 검증한다"
  ...
  - "GPS 권한 거부 또는 위치 품질 저하 시 앱은 기록 불가/품질 저하 경고를 표시한다. 여기서 위치 품질 저하는 정확도 저하, 시간 오차, 비정상 속도/점프처럼 샘플 자체의 신뢰도 문제이며, 담당 구역 안팎 여부를 품질 저하로 보지 않는다"
