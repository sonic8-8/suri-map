...
- **then**:
  ...
  6. 신규 배정된 지원 부대의 `TEAM_PHONE`, `PATROL_CAR_PHONE` 등록 단말에 PII 없는 사건 배정 FCM data message가 도달한다. 지휘 계정 deviceId는 Android FCM recipient로 고정하지 않는다.
...
- **e2e_red_test**:
  ...
  - "지원 부대 배정 시 `INCIDENT_MEMBERSHIP_CHANGED` fanout이 신규 배정된 지원 부대 업무폰 FCM recipient와 PII 없는 payload를 mock dispatcher에 기록한다"
...
- **then**:
  1. 화면에는 완료 상태가 표시되고, 저장 상태는 `FIRST_SEARCH_COMPLETED` 또는 `RECHECK_COMPLETED`로 변경되며 `AREA_STATE_CHANGED`가 발행된다.
  2. `search_area_history`에 처리 계정, 시각, 현재 OP, 메모가 기록된다.
  3. 앱과 웹에 완료 상태가 반영된다.
  4. 새 OP 생성 시 사유(`SHIFT_CHANGE`, `RE_SEARCH`, `NEW_AREA`, `OTHER`)가 기록되고 `OP_TRANSITIONED`가 발행된다.
