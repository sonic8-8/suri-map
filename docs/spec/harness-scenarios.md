...
- Spec 오너십은 정적이다. 예: 전체 수색 구역과 구역은 S2, OP와 인수인계는 S8.
...
- 개별 Lane 하네스 PASS는 자기 계약의 PASS다. SC 최종 PASS는 `boundaries.md` §10 Harness Mapping에 걸린 모든 Lane 계약과 S4 event 수렴, S3-2 board response 수렴까지 통과해야 한다.
- 다른 Spec 엔티티를 변경할 때는 해당 Spec 오너 LGTM이 필요하다.
- S4 이벤트 허브 실구현은 L2가 주도한다. 계약 변경은 전 Lane LGTM이지만 SSE 엔드포인트, `EventHub.publish`, `sse_replay_event` 개발·유지보수·버그 수정은 L2 담당이다.
- 상황판 웹 UI 기여 규칙:
...
`involved_specs`는 해당 시나리오가 직접 검증하는 계약·데이터·이벤트 오너를 적는다. 상황판 slot 통합은 `board_merge`에 별도로 적고, S3-2 shell·API assembly·board response 자체가 시나리오의 주요 검증 대상일 때만 S3-2를 `involved_specs`에도 포함한다. 단순 slot mount 경유는 `board_merge`와 `boundaries.md` §10 Harness Mapping에서 추적한다.

### 0.3 write 이벤트·상황판 수렴 공통 red test

SC-05, SC-06, SC-08, SC-10, SC-11, SC-12의 write 성공 red test는 아래 대조를 공통으로 포함한다.

- REST 응답의 `id`, `status`, `version`이 mock event_dispatch_job row, SSE payload, board response row와 같은 사건·OP·PolicePhone·entity 상태를 말해야 한다.
...
- write 성공 후 board API refetch/assembly는 REST 응답 `version` 이상으로 수렴해야 한다. board response row가 없거나 낮은 `version`의 stale board response를 반환하면 red test 실패다.
- 실패 주입 red test는 REST DB commit 성공 + `event_dispatch_job` 미적재, `event_dispatch_job` 적재 + SSE 미송신, FCM 대상 시나리오의 mock FCM dispatcher 미수신, board API refetch/assembly 미갱신을 각각 실패로 판정해야 한다.
...
| SC-04 | 전체 수색 구역·구역 분할·할당 | 3 | ○ |
| SC-05 | 수색 경로·PolicePhone GPS 경로 | 4 | ○ |
| SC-06 | 현장 마커 생성 | 5 | ○ |
| SC-07 | 통신 단절 중 로컬 기록 | 4 | - |
| SC-08 | 지원 요청·실종자 발견 알림 | 5 | ○ |
| SC-09 | 통신 복구·동기화 | 4 | ○ |
| SC-10 | 구역 완료·새 OP 열기 | 3 | ○ |
| SC-11 | 인수인계·OP 비교·수색 이력 요약 | 6 | ○ |
| SC-12 | 사건 종료·데이터 파기 | 1 | ○ |
...
- **then**:
  ...
  3. 적재 상태가 서버에 보고되고 `OFFLINE_PACKAGE_INSTALLATION_CHANGED`가 발행된다.
  4. 적재 미완료 단말은 상황판에 경고 배지로 표시된다.
  5. 지구대/파출소 초동 대응 단말은 담당 구역이 아직 없더라도 전체 수색 구역과 초기 기준 마커를 받아 순찰차/도보 수색을 시작할 수 있다.
  6. 앱은 실패한 항목명과 재시도 대상만 표시한다. 지도 타일 미다운로드 상태는 오프라인 지도 준비 경고로 표시하되 사건 확인과 현장 기록 화면 진입은 차단하지 않는다.
...
- **e2e_red_test**:
  ...
  - "실패 항목이 남은 패키지는 오프라인 사용 준비 완료, 서버 반영 완료 상태로 전환되지 않지만 사건 확인과 현장 기록 화면 진입은 차단하지 않는다"
  - "만료된 manifest로 패키지 적재 상태 보고를 시도하면 앱은 만료/재다운로드 필요 상태를 표시하고 오프라인 사용 준비 완료, 서버 반영 완료 상태로 전환하지 않지만 사건 확인과 현장 기록 화면 진입은 차단하지 않는다"
  - "전체 수색 구역 변경 후 기존 패키지는 stale package로 표시되고 오프라인 사용 준비 완료, 서버 반영 완료 상태로 유지되지 않지만 사건 확인과 현장 기록 화면 진입은 차단하지 않는다"
...
- **notes**: 전체 수색 구역는 도심·외곽·논밭·하천·산악을 모두 포괄하는 범용 범위 개념이다.
...
- **then**:
  ...
  3. `search_area_assignment`가 현재 OP 기준으로 추가되고 `SEARCH_AREA_ASSIGNMENT_CHANGED`가 발행된다.
...
- **e2e_red_test**:
  ...
  - "`search_area` 생성·분할·배정은 기존 `overall_search_area`와 오프라인 패키지 manifest의 범위를 변경하지 않는다"
  - "전체 수색 구역·구역 저장 중에는 저장 버튼이 로딩·비활성 상태가 되고 중복 저장되지 않는다"
  - "전체 수색 구역과 수색 구역은 유효한 Polygon으로만 저장되고 자기 교차·빈 geometry는 거부된다"
  - "`overall_search_area` 저장 payload의 geometry type이 Polygon이 아니면 `400 invalid_geometry`를 응답하고 overall_search_area row, event_dispatch_job, board response를 변경하지 않는다"
  - "`overall_search_area` 저장 성공 시 좌표는 EPSG:4326 `[lon, lat]`, ring 폐합, 연속 중복점 제거, 소수 6자리 precision으로 정규화되어 REST 응답·DB row·board response가 같은 canonical geometry를 가진다"
  - "`overall_search_area` 좌표가 하네스 기준 지도 envelope 밖이거나 경도/위도 범위를 이탈하면 `400 invalid_geometry`를 응답하고 draft만 유지한다"
  - "`overall_search_area` Polygon이 0면적 또는 하네스 최소 면적 미만, 미폐합 ring, 자기교차, 빈 ring이면 `400 invalid_geometry`를 응답하고 기존 active overall_search_area를 유지한다"
  - "`overall_search_area` geometry의 CRS가 EPSG:4326이 아니거나 허용 precision을 초과하는 좌표가 정규화 후에도 기준을 만족하지 못하면 `400 invalid_geometry`를 응답한다"
  - "`search_area` 생성·분할 payload의 geometry type이 Polygon이 아니면 `400 invalid_geometry`를 응답하고 area row, assignment, event_dispatch_job, board response를 변경하지 않는다"
  - "`search_area` 저장 성공 시 좌표는 EPSG:4326 `[lon, lat]`, ring 폐합, 연속 중복점 제거, 소수 6자리 precision으로 정규화되어 REST 응답·DB row·board response가 같은 canonical geometry를 가진다"
  - "`search_area` 좌표 또는 bbox가 현재 active `overall_search_area` 밖이거나 경도/위도 범위를 이탈하면 `400 invalid_geometry`를 응답하고 구역·배정 draft만 유지한다"
  ...
  - "앱에서 전체 수색 구역 변경 API를 호출하면 `403 channel_not_allowed`"
  - "앱에서 구역 생성·분할 API를 호출하면 `403 channel_not_allowed`"
  - "앱에서 OP 담당 구역 배정 API를 호출하면 `403 channel_not_allowed`"
  - "전체 수색 구역·구역 변경 후 상황판은 변경된 영역을 즉시 강조하고 담당 단말에는 새 구역 배정 상태를 표시한다"
- **board_merge**: `overall_search_area` slot + `area` slot
...
- **e2e_red_test**:
  ...
  - "`search_path` 포인트가 현재 `overall_search_area` 또는 하네스 기준 지도 envelope 밖이거나 경도/위도 범위를 이탈하면 `400 invalid_geometry`로 해당 batch 전체를 거부하고 partial path write를 남기지 않는다"
...
- **e2e_red_test**:
  ...
  - "마커 생성 시 board response에는 marker slot/Point 표시만 추가되고 `search_path`, `search_area`, `op_toggle` row는 생성·변경되지 않는다"
  - "`marker` 생성 payload의 geometry type이 Point가 아니면 `400 invalid_geometry`를 응답하고 marker row, photo row, event_dispatch_job, board response를 변경하지 않는다"
  - "`marker` 저장 성공 시 좌표는 EPSG:4326 `[lon, lat]`, 소수 6자리 precision으로 정규화되어 REST 응답·DB row·board response가 같은 canonical Point를 가진다"
  - "`marker` Point가 현재 `overall_search_area` 또는 담당 `search_area` 밖이어도 좌표 자체가 유효한 EPSG:4326 Point이면 온라인 서버는 marker row와 event_dispatch_job을 생성하고, 오프라인 앱은 replay 성공 시 pending 마커를 서버 반영 완료로 바꾼다"
  ...
  - "사진 업로드용 presigned URL 발급, upload, attach 단계 실패 시 앱은 실패 단계와 사유를 표시하고 업로드 재시도 CTA를 제공한다"
...
- **given**: 사건 `OPEN`, SC-03에서 오프라인 패키지 준비 완료 상태로 앱 수색 중 네트워크가 30분 이상 단절. SC-03 패키지 미완료·만료·stale package 단말은 오프라인 지도 사용 가능 상태와 구분하되 사건 확인과 현장 기록 화면 진입 자체를 차단하지 않는다.
- **when**: 단절 상태에서 경로 수집과 마커 생성을 계속 수행
- **then**:
  1. 경로·마커·사진 임시 파일이 단말 로컬 DB와 Outbox에 저장된다.
  2. 앱은 "오프라인 / 기록 중" 상태와 미전송 큐 수량을 표시한다.
  3. GPS 중단, 배터리 저하 경고는 서버 의존 없이 로컬 상시 경고로 표시되고, 지도 미다운로드는 현장 진입 시 일시 안내로 표시된다.
  4. 서버 이벤트는 발행되지 않는다.
  5. 패키지 미완료·만료·stale package 상태에서는 오프라인 지도 사용 가능 상태로 표시하지 않고 현장 진입 시 일시 안내를 표시하되 현장 기록 화면 진입과 로컬 기록 저장은 차단하지 않는다.
...
- **e2e_red_test**:
  ...
  - "패키지 준비 완료 상태에서 오프라인 지도 사용 중 로컬 경고가 발생하면 서버 호출 없이 앱 경고와 Outbox 적재 상태가 함께 유지된다"
  - "패키지 미완료·만료·stale package 상태에서 네트워크가 단절되면 앱은 오프라인 지도 사용 가능 상태로 전환하지 않고 현장 진입 시 미완료/재적재 필요 안내를 일시 표시하되 현장 기록 화면 진입과 로컬 기록 저장은 차단하지 않는다"
  - "패키지 준비 -> 오프라인 지도 사용 -> 로컬 경고/Outbox 적재 -> SC-09 복구 동기화 순서의 상태 전이가 끊기지 않는다"
  - "로컬 저장 실패 시 앱은 기록 불가 사유와 재시도 또는 계속 기록 불가 상태를 표시한다"
  - "앱 재시작 후에도 미전송 큐가 유지된다"
  - "지도 미다운로드 상태에서 현장 진입 시 `오프라인 지도가 준비되지 않았어요` / `오프라인 사용 전 다운로드가 필요합니다` 로컬 안내가 토스트처럼 일시 표시되고 사라진다"
- **board_merge**: -
...
- **e2e_red_test**:
  ...
  - "지원 요청 알림은 웹 토스트·앱 배너·FCM data message 기반 백그라운드 OS notification에서 생성 계정/팀, PolicePhone 이름 또는 유형, OP, 마커 유형, 위치 요약을 함께 표시한다"
  - "실종자 발견 알림은 지원 요청과 다른 중요도·문구뿐 아니라 생성 출처 계정/PolicePhone/OP를 표시한다"
...
- **then**:
  ...
  4. `PATH_APPENDED`, `MARKER_CREATED`, `OFFLINE_PACKAGE_INSTALLATION_CHANGED` 등 필요한 이벤트가 서버 수신 기준으로 발행된다.
...
- **then**:
  1. 화면에는 완료 상태가 표시되고, 저장 상태는 `FIRST_SEARCH_COMPLETED` 또는 `RECHECK_COMPLETED`로 변경되며 `SEARCH_AREA_CHANGED`가 발행된다.
  2. `search_area_history`에 처리 계정, 시각, 현재 OP, 메모가 기록된다.
  3. 앱과 웹에 완료 상태가 반영된다.
  4. 새 OP 생성 시 사유(`RE_SEARCH`, `AREA_CHANGED`, `OTHER`)가 기록되고 `OP_TRANSITIONED`가 발행된다.
  5. 근무 교대는 `duty_shift`로 기록하고, 지구대/파출소 팀장 또는 당직자는 초동 대응 OP1에서 수색 완료 보고를 반영한 뒤 실종팀 인계 등 별도 사유가 필요할 때 `OTHER`와 인수인계 메모를 남기며 OP2를 열 수 있다.
...
- **e2e_red_test**:
  ...
  - "구역 완료, OP 전환, 인수인계 메모 write는 §0.3 공통 red test에 따라 REST 응답 id/status/version, `event_dispatch_job`, SSE payload, board response area/op/handover row가 같은 완료·현재 OP·메모 상태를 말하고 board response version이 수렴해야 한다"
...
- **then**:
  ...
  2. 사용자 조회용 `missing_person`/사진 데이터가 tombstone 응답에서 제거된다.
  3. 단말은 동기화 완료 확인 후 사건 패키지, 지도 타일/manifest, 로컬 경로, 로컬 마커, 임시 사진, 개인정보 로컬 데이터를 삭제한다.
...
- **e2e_red_test**:
  ...
  - "`INCIDENT_CLOSED` 수신 후 앱은 사건 로컬 상태를 닫고 후속 FCM data message를 새 작업으로 처리하지 않으며, 웹 상황판은 SSE incident stream을 닫아 이후 같은 사건의 실시간 이벤트를 수신·렌더링하지 않는다"
  ...
  - "단말 동기화 완료 후 실종자 개인정보·사진 로컬 데이터와 위치 로컬 데이터가 제거된다"
  ...
  - "위치정보 접근 기록과 운영 기록 같은 내부 보존 기록은 사용자 UI, 앱 로컬 데이터, 오프라인 패키지, 상황판 조회 응답에 노출되지 않는다"
  - "사용자 삭제 대상인 개인정보·위치·사진·오프라인 패키지와 내부 보존 대상인 운영 기록은 서로 다른 저장소/fixture로 검증되고 사용자 삭제 처리로 내부 보존 기록이 사용자 조회 가능 상태가 되지 않는다"
  - "사건 종료 write는 §0.3 공통 red test에 따라 REST 응답 id/status/version, `event_dispatch_job`, SSE payload, board response incident row가 같은 terminal/closed 상태를 말하고 board response version이 수렴해야 한다"
- **board_merge**: `incident_terminal` slot + `package_badge` slot + tombstone response 조회
- **notes**: 운영 로그·접속기록은 사용자 화면 기능이 아니다. 내부 보존은 UI·export 요구가 아니라 사용자 UI, 앱 로컬 데이터, 오프라인 패키지, 상황판 조회에 노출되지 않는다는 검증 대상으로만 다룬다. 사건 종료 시나리오의 검증 대상은 개인정보 로컬 데이터와 단말 로컬 데이터 파기다.
...
- S1-2 계정·PolicePhone·권한은 모든 API 시나리오의 공통 전제다.
...
1. S1-2: team/account/policePhone fixture와 channel/role matrix
2. S4: event payload envelope, `event_dispatch_job`, SSE replay 최소 계약
3. S3-2: board shell slots, slot props, board API 수렴 기준
...
| 하네스 geometry/GPS 기준 좌표 | SC-04, SC-05, SC-06 | 기준 지도 envelope는 EPSG:4326 bbox `minLon=126.647507`, `minLat=35.052595`, `maxLon=127.017482`, `maxLat=35.256837`이다. 최소 Polygon 면적은 `400m2`이고 좌표 precision은 소수 6자리 canonical 값을 기준으로 한다. 정상 `overall_search_area` fixture는 `[[126.904000,35.158000],[126.923000,35.158000],[126.923000,35.173000],[126.904000,35.173000],[126.904000,35.158000]]`, 정상 `search_area` fixture는 `[[126.910000,35.160000],[126.918000,35.160000],[126.918000,35.166000],[126.910000,35.166000],[126.910000,35.160000]]`, 정상 marker fixture는 `[126.913400,35.163100]`를 쓴다. 구역 밖 marker 허용 fixture는 `coord-outside-envelope=[127.200000,35.163100]`이고, 실패 좌표 fixture는 `coord-latlon-swapped=[35.163100,126.913400]`, `polygon-unclosed`, `polygon-self-intersecting`, `polygon-too-small-under-400m2`, `point-null-nan`, `precision-over-6dp=[126.9134007,35.1631007]`로 고정한다. SC-04/05는 Polygon/Path 구역 포함 검증을 유지하고, SC-06 marker geometry test는 좌표 자체의 유효성만 `invalid_geometry`로 본다 |
...
| mock SC-10 command flow | SC-10 | mock radio report는 `radioReportId=rr-precinct-001`, `incidentAlias=inc-precinct-first-001`, `incidentId=aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaa0001`, `opAlias=op-precinct-001-op1`, `opId=88888888-8888-8888-8888-888888880001`, `areaAlias=area-precinct-a1`, `areaId=cccccccc-cccc-cccc-cccc-cccccccc0001`, `reportedByAccountCode=acct-precinct-cmd`, `message=1구역 수색 완료, 북측 미확인 동선 존재`, `receivedAt=2026-04-28T10:15:00+09:00`를 쓴다. commander decision fixture는 `decisionId=dec-op-transition-001`, `decision=COMPLETE_AREA_AND_OPEN_NEXT_OP`, `nextOpAlias=op-precinct-001-op2`, `nextOpId=88888888-8888-8888-8888-888888880002`, OP1 seed 보존용 `handoverMemoId=memo-precinct-handover-001`를 쓴다. SC-11 OP2 메모 write/convergence는 `S8.json`의 `memo-precinct-op2-001`를 별도 fixture로 쓴다. 실행 로그 저장소는 `harness_execution_log`이며 검증 키는 `incidentId`, `opId`, `areaId`, `radioReportId`, `decisionId`, `eventId`, `sequence`, `actorAccountId`, `createdAt`이다. 기대 event/log sequence는 `radio_report_received -> commander_decision_recorded -> area_completed -> op_transitioned -> handover_saved -> board_api_refetched`이고, 누락·순서 역전·다른 사건/OP 키 혼입은 red test 실패다 |
| mock search history summary provider | SC-11 | 성공, 실패, 금지 문장 포함 응답을 재현 |
