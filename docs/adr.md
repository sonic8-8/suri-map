...
- 개발·하네스 object storage adapter는 MinIO(S3-compatible)를 기본으로 하고, harness는 mock object storage/upload URL(presigned URL) fixture를 사용한다. 운영 배포는 S3-compatible port를 통해 S3로 전환한다.
