...
| `photos/upload-url`, `photos/{photoId}/attach` | API 용어는 upload URL/attach로 유지한다. 여기서 `uploadUrl`은 S3-compatible object storage presigned URL이다. harness의 `presign`/`finalize` 표현은 endpoint 이름으로 쓰지 않는다. |
...
- Note: response의 `uploadUrl`은 S3/MinIO-compatible presigned URL이다. API endpoint 이름은 storage 구현 용어인 `presign`이 아니라 클라이언트 동작인 `upload-url`로 둔다.
