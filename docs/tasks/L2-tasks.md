# L2 Tasks · Account, Device, RBAC, Retention, Event Hub

상태: 초안. 담당 Lane: L2. 담당 Spec: S1-2, S1-3, S4. 주요 하네스: auth/device, event hub, purge orchestration.

## 작업 범위

- account, role, team, device, session, channel guard
- FCM token registration and device freshness
- event envelope, event outbox, SSE replay, `EventHub.publish`
- retention purge orchestration and internal operational records

## 외부 계약

- Provides auth/device fixture to all lanes.
- Provides S4 event contract to all domain lanes.
- Provides purge orchestration hooks to L1, L4, L5, L6.
- Provides device freshness input to L6/S3-2.
...
- [ ] L2-B03A 이벤트 테스트 기반과 모의 발행 후크 작성
  ...
  - 예상 작업량: 1d
  - 완료 기준: real outbox 또는 SSE dispatch 구현 전에 안정적인 mock publish hook 기준으로 event test를 작성할 수 있다.
...
- [ ] L2-T01 계정·역할·단말 테스트 데이터 기준 준비
  ...
  - 구현 산출물: account/team/device fixture, role/channel matrix fixture, test seed loader, common auth mock
  - 예상 작업량: 1d
  - 완료 기준: 실종팀, 순찰차, 지휘관, 지원 부대, 지구대/파출소 account와 device ID가 `spec/harness-scenarios.md` fixture ID와 일치한다.
...
- [ ] L2-T02 채널·역할 권한 별칭 구현
  ...
  - 예상 작업량: 1d
  - 완료 기준: `public-session`, `incident-read`, `web-command`, `app-device`, `field-or-web-write`가 `spec/boundaries.md §7` guard shorthand와 일치하고, `write-common`은 S1-1 incident lifecycle과 S6 idempotency 계약으로 위임되며 guard 실패 시 domain row를 만들지 않는다.

- [ ] L2-T03 단말 배정 fixture와 heartbeat 상태 보고 구현
  - 담당 Spec: S1-2
  - 필수 참조: `spec/specs/S1-2.json`, `spec/boundaries.md §3 S1-2`, `spec/boundaries.md §9.2 device_freshness`
  - 연관 Spec: S3-1, S6, S7, S3-2
  - 시나리오: SC-03, SC-05, SC-07, SC-09
  - 관련 FR: FR-02, FR-24, FR-25
  - 구현 산출물: device assignment fixture state, assignment guard/service, `POST /devices/{deviceId}/heartbeat` write path, `DEVICE_HEARTBEAT_UPDATED` publish request, DeviceFreshnessQuery fixture
  - 예상 작업량: 2d
  - 완료 기준: 배정된 device fixture는 package 조회와 현장 기록 write가 가능하고, 미배정 device는 assignment guard로 실패하며, 새 public device registration API 없이 heartbeat가 device freshness projection에 반영된다.
...
- [ ] L2-T05 S4 기본 이벤트 포맷 검증 구현
  ...
  - 예상 작업량: 1d
  - 완료 기준: real `event_outbox` 구현 전에도 모든 domain publish request가 mock contract에서 base envelope, schema version, id/status/version 필드, owner payload schema 기준으로 검증된다.
...
- [ ] L2-T06 이벤트 저장·발행 트랜잭션 경계 구현
  ...
  - 관련 FR: FR-04, FR-10, FR-18, FR-24
  - 선행 task: L2-T05
  - 구현 산출물: event_outbox entity/migration, `EventHub.publish` port, publish transaction tests, outbox ownership tests
  - 예상 작업량: 2d
  - 완료 기준: domain transaction은 publish request를 원자적으로 stage할 수 있고, `event_outbox` row와 dispatch state 소유권은 S4에 남는다.
...
- [ ] L2-T07A SSE endpoint와 Last-Event-ID 재전송 구현
  ...
  - 관련 FR: FR-04, FR-10, FR-18, FR-24
  - 구현 산출물: Spring MVC `SseEmitter` 기반 `GET /events` SSE endpoint, `sse_event_log`, Last-Event-ID replay tests
  - 예상 작업량: 1d
  - 완료 기준: `GET /events?incidentId=...`가 Web/S3-2 consumer에 대해 live send와 `sse_event_log` 기반 Last-Event-ID replay를 지원하고, Android APP channel은 SSE consumer가 아니다.

## Phase 3

- [ ] L2-T07B SSE 중복·순서 뒤바뀜·snapshot 복구 실패 주입 구현
  - 담당 Spec: S4
  - 필수 참조: `spec/specs/S4.json`, `spec/harness-scenarios.md §0.3`, `spec/harness-scenarios.md §6 mock SSE client`
  - 연관 Spec: S1-1, S1-2, S1-3, S2, S3-1, S5, S6, S7, S8, S3-2
  - 시나리오: SC-05, SC-06, SC-08, SC-09, SC-10, SC-11, SC-12
  - 관련 FR: FR-04, FR-10, FR-18, FR-24
  - 선행 task: L2-T07A
  - 구현 산출물: duplicate event failure injection test, reordered event test, gone snapshot recovery test
  - 예상 작업량: 1d
  - 완료 기준: duplicate/reordered event test와 gone snapshot recovery check가 SSE replay path 기준으로 통과한다.
...
- [ ] L2-T04 FCM 토큰 등록 fixture와 활성 토큰 조회 구현
  ...
  - 시나리오: SC-02, SC-08
  - 관련 FR: FR-24
  - 지원 FR: FR-10, FR-17 through S5 notification delivery
  - 구현 산출물: FCM token registration fixture, `FcmTokenQuery.activeByDevice(deviceId)` fixture, logout/token lifecycle fixture
  - 예상 작업량: 1d
  - 완료 기준: S5가 notification delivery를 위해 `FcmTokenQuery.activeByDevice(deviceId)` fixture를 소비할 수 있고, L2는 incident-scoped unsubscribe/invalidate를 소유하지 않는다.
...
- [ ] L2-T08 데이터 삭제 오케스트레이션과 운영 기록 구현
  ...
  - 시나리오: SC-12
  - 관련 FR: FR-22
  - 구현 산출물: purge_run entity/migration, purge hook coordinator, location_access_log/internal log sink contract, `INCIDENT_PURGED` publish request tests
  - 예상 작업량: 2d
  - 완료 기준: `INCIDENT_CLOSED`가 purge run을 시작하고 `spec/boundaries.md §4.5` lifecycle 순서로 hook을 조율하며, internal-only 운영 기록을 저장하고 모든 hook 성공 후에만 sanitized purge/tombstone state를 발행한다.
...
- [ ] L2-T07C 사건 종료 SSE 재전송 중지와 stream 해제 검증 구현
  ...
  - 예상 작업량: 1d
  - 완료 기준: `INCIDENT_CLOSED` 후 같은 사건 SSE stream은 해제되고, `INCIDENT_PURGED` 후 old replay data가 재전송되지 않으며, S3-2 board snapshot 수렴 evidence가 남는다.
...
- [ ] L2-T09A 인증·단말 모의 계약 하네스 작성
  ...
  - 연관 Lane: All lanes
  - 시나리오: auth/device paths across SCs
  - 구현 산출물: auth/device harness runner, role/channel mocks, device assignment mocks, mock-to-real auth contract swap tests
  - 예상 작업량: 1d
  - 완료 기준: 다른 Lane이 L2 mock으로 auth/device path를 실행하고 fixture ID 변경 없이 real S1-2 contract로 교체할 수 있다.

- [ ] L2-T09B 이벤트 허브 모의 계약 하네스 작성
  - 담당 Spec: S4
  - 필수 참조: `spec/specs/S4.json`, `spec/harness-scenarios.md §0.3`, `spec/harness-scenarios.md §6 mock event outbox`
  ...
  - 예상 작업량: 1d
  - 완료 기준: event-producing Lane이 S4 mock으로 실행되고 이후 real event envelope/outbox/SSE contract로 교체할 수 있다.
...
- Offline package manifest/status domain logic
