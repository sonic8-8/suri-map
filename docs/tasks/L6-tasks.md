...
- board slot registry, projection, snapshot read model
- offline package manifest/status, package badge, tileserver endpoints
...
- Consumes L2/S4 SSE/projector trigger contract.
- Consumes L3/S2 boundary/area query and L3/S8 OP/handover query.
- Consumes L5/S5 marker query and L4/S3-1 path query.
- Provides board slot registry and snapshot convergence expectations to all lanes.
...
- [ ] L6-T05 오프라인 패키지 목록 계약 fixture 작성
  ...
  - 시나리오: SC-03
  - 관련 FR: FR-21, FR-31
  - 구현 산출물: offline_package_manifest contract fixture, manifest fixture builder, missing person/OP/area/marker/boundary/tile item contract tests, SC-03 tile fixture exactness tests
  - 예상 작업량: 1d
  - 완료 기준: real package API 구현 전에 Phase 0 fixture가 incident metadata, missing person cache, OP, assigned area, initial marker, map boundary, tile reference, device context를 담고, tile fixture는 `tile-manifest-inc-precinct-001-v1`, zoom `15..16`, `local://tiles/inc-precinct-first-001/{z}/{x}/{y}.pbf` 기준으로 검증된다.
...
- [ ] L6-T03A 지도 기준 범위·수색 구역 slot 구현
  - 담당 Spec: S3-2
  - 필수 참조: `spec/specs/S3-2.json`, `spec/boundaries.md §9.2`, `spec/boundaries.md §10`
  - 연관 Spec: S2
  - 시나리오: SC-04
  - 관련 FR: FR-05, FR-07
  - 구현 산출물: map_boundary slot renderer, area slot renderer, S2 source-owner immutability tests
  - 예상 작업량: 1d
  - 완료 기준: boundary/area slot이 source owner를 변경하지 않고 S2 source data를 렌더링하며 관찰 가능한 loading/failure state를 노출한다.
...
- [ ] L6-T03C 단말 최신성 슬롯과 공통 지연 상태 검증
  ...
  - 시나리오: SC-05, SC-09
  - 관련 FR: FR-24, FR-25
  - 구현 산출물: device_freshness slot renderer, common stale/loading/failure state tests, stale badge fixture
  - 예상 작업량: 1d
  - 완료 기준: source owner data를 변경하지 않고 device freshness slot과 shared stale/loading/failure state가 표시된다.
...
- [ ] L6-T02A 상황판 스냅샷 투영 모델 구현
  - 담당 Spec: S3-2
  - 필수 참조: `spec/specs/S3-2.json`, `spec/specs/S4.json`, `spec/harness-scenarios.md §0.3`, `spec/harness-scenarios.md §6 mock board snapshot projector`
  - 연관 Spec: S1-1, S1-2, S1-3, S2, S3-1, S4, S5, S6, S7, S8
  - 시나리오: SC-04, SC-05, SC-06, SC-08, SC-09, SC-10, SC-11, SC-12
  - 구현 산출물: board_snapshot projection model, owner response version mapping tests, projection row fixture
  - 예상 작업량: 1d
  - 완료 기준: source owner data를 변경하지 않고 owner response version에서 snapshot row를 만들 수 있다.

- [ ] L6-T02B eventId 중복 제거와 오래된 이벤트 차단 구현
  - 담당 Spec: S3-2
  ...
  - 예상 작업량: 1d
  - 완료 기준: duplicate/stale event는 무시되고 더 최신 owner version만 snapshot을 갱신한다.

- [ ] L6-T02C SSE 수렴과 투영 지연 관측 검증 구현
  - 담당 Spec: S3-2
  - 필수 참조: `spec/specs/S3-2.json`, `spec/specs/S4.json`, `spec/harness-scenarios.md §0.3`, `spec/harness-scenarios.md §6 mock board snapshot projector`
  - 연관 Spec: S4
  - 시나리오: SC-04, SC-05, SC-06, SC-08, SC-09, SC-10, SC-11, SC-12
  - 구현 산출물: SSE convergence tests, projection lag state, lag observation fixture
  - 예상 작업량: 1d
  - 완료 기준: SSE event 이후 board snapshot이 수렴하고, 수렴이 지연될 때 projection lag state를 관찰할 수 있다.

- [ ] L6-T06A offline_package_status API·이벤트·조회 구현
  - 담당 Spec: S7
  ...
  - 구현 산출물: offline_package_status API test, `PACKAGE_STATUS_CHANGED` publish request test, PackageStatusQuery fixture/contract test
  - 예상 작업량: 1d
  - 완료 기준: status report API test가 manifest item state를 갱신하고, publish request test가 `PACKAGE_STATUS_CHANGED`를 capture하며, PackageStatusQuery contract test가 기대 package status field를 노출한다.

- [ ] L6-T06B package_badge 투영과 S6 로컬 경고 입력 검증
  - 담당 Spec: S7, S3-2
  - 필수 참조: `spec/specs/S7.json`, `spec/specs/S6.json`, `spec/specs/S3-2.json`
  - 연관 Spec: S3-2, S6
  - 시나리오: SC-03, SC-07, SC-09, SC-12
  - 관련 FR: FR-19, FR-29, FR-31
  - 구현 산출물: package_badge projection tests, S6 local warning input fixture, package unavailable/stale evidence
  - 예상 작업량: 1d
  - 완료 기준: package status가 board package badge로 projection되고 unavailable/stale package에 대한 S6 local warning input을 제공한다.
...
- [ ] L6-T04A OP 비교·이력·인수인계 slot 구현
  ...
  - 구현 산출물: op_toggle/op_history/handover_memo/handover_status slot renderers, fallback UI state, S8 source-owner immutability tests
  - 예상 작업량: 1d
  - 완료 기준: S8 source data를 변경하지 않고 OP toggle/history와 handover slot이 source evidence와 fallback state를 표시한다.

- [ ] L6-T04B AI 요약 대체 표시와 FR-23 금지 행동 버튼 차단 구현
  - 담당 Spec: S3-2
  ...
  - 시나리오: SC-11
  - 관련 FR: FR-23, FR-39
  - 구현 산출물: ai_summary slot renderer, OpenAI-generated summary success state, loading state, fallback/unavailable UI state, source evidence links, forbidden recommendation CTA tests
  - 예상 작업량: 1d
  - 완료 기준: AI summary slot이 OpenAI-generated 요약과 fallback/unavailable 상태를 구분해 표시하고, source prompt/provider secret을 노출하지 않으며 자동 누락 구역 확정, 위험도 판단, 다음 구역 CTA를 차단한다.

- [ ] L6-T07 지도 기준 범위 버전과 만료 패키지 처리 구현
  - 담당 Spec: S7
  - 필수 참조: `spec/specs/S7.json`, `spec/specs/S2.json`, `spec/boundaries.md §10 SC-03`, `spec/boundaries.md §10 SC-04`
  - 연관 Spec: S2, S4, S3-2
  - 시나리오: SC-03, SC-04
  - 관련 FR: FR-31
  - 구현 산출물: manifest revision policy, stale package transition, `MAP_BOUNDARY_CHANGED` consumer test, re-download requirement UI state
  - 예상 작업량: 1d
  - 완료 기준: `MAP_BOUNDARY_CHANGED`가 manifest revision을 증가시키고 기존 READY/PARTIAL package status를 STALE로 표시하며 re-download requirement를 노출한다.

- [ ] L6-T09A offline_package_manifest/offline_package_status 삭제 후크와 삭제 표식 전이 구현
  - 담당 Spec: S3-2, S7
  ...
  - 시나리오: SC-12
  - 관련 FR: FR-31
  - 지원 FR: FR-22 through sanitized terminal/package purge projection
  - 선행 task: L1-T06, L2-T08
  - 구현 산출물: package purge hook, package status tombstone transition, S1-3 purge hook contract test
  - 예상 작업량: 1d
  - 완료 기준: package manifest/status purge hook이 실행되고 missing person package data를 보존하지 않은 채 package state를 sanitized tombstone으로 전이한다.

- [ ] L6-T09B incident_terminal/package_badge 투영과 개인정보 제거 검증
  - 담당 Spec: S3-2, S7
  - 필수 참조: `spec/specs/S3-2.json`, `spec/specs/S7.json`, `spec/specs/S1-3.json`, `spec/harness-scenarios.md §2 SC-12`
  - 연관 Spec: S1-1, S1-3, S4, S6
  - 시나리오: SC-12
  ...
  - 구현 산출물: terminal/tombstone projection, incident_terminal/package_badge board states, missing person data absence tests
  - 예상 작업량: 1d
  - 완료 기준: board가 sanitized terminal/package badge state를 표시하고 terminal projection에 missing person data가 남지 않는다.
...
- [ ] L6-T10B 상황판 snapshot/SSE 수렴 하네스 작성
  - 담당 Spec: S3-2
  - 필수 참조: `spec/specs/S3-2.json`, `spec/specs/S4.json`, `spec/harness-scenarios.md §0.3`
  - 연관 Lane: All event-producing lanes
  - 시나리오: board convergence for SC-02 through SC-12
  - 구현 산출물: board snapshot harness runner, SSE convergence tests, slot merge tests, projection lag tests
  - 예상 작업량: 1d
  - 완료 기준: board snapshot, slot merge, SSE convergence, projection lag test가 mocked source contract 기준으로 통과한다.

- [ ] L6-T10C SC-11 OP·AI 요약·FR-23 렌더링 차단 하네스 작성
  - 담당 Spec: S3-2
  - 필수 참조: `spec/specs/S3-2.json`, `spec/specs/S8.json`, `spec/boundaries.md §6 FR-23`, `spec/harness-scenarios.md §2 SC-11`, `adr.md ADR-0034`
  - 연관 Lane: L3, L4, L5
  - 시나리오: SC-11
  - 구현 산출물: OP/AI summary harness runner, OpenAI success/fallback slot fixtures, FR-23 rendering guard tests, forbidden CTA evidence
  - 예상 작업량: 1d
  - 완료 기준: OP/AI summary rendering guard test가 OpenAI success/fallback 상태 모두에서 통과하고 금지된 recommendation/risk/missing-area CTA state가 숨겨진 상태로 유지된다.
...
- Event outbox/SSE dispatch internals
