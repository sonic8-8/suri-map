# L1 Tasks · Incident Import & Membership

상태: 초안. 담당 Lane: L1. 담당 Spec: S1-1. 주요 SC: SC-01, SC-02, SC-12.

## 작업 범위

- mock/seed 배정 사건 가져오기
- incident lifecycle: `BOOTSTRAPPING`, `OPEN`, terminal close
- missing person operational cache
- incident membership and commander assignment
- incident list/detail DTO
- close request handoff to purge and terminal snapshot

## 외부 계약

- Consumes L2/S1-2 auth, role, device, incident access fixture.
- Publishes through L2/S4 event envelope and `EventHub.publish`.
- Calls L5/S5 initial marker seed port.
- Triggers L3/S8 OP1 bootstrap through `INCIDENT_CREATED`.
- Hands close/purge to L2/S1-3 and cache/package removal hooks.

## Phase -1

- [ ] L1-B01 배정 사건·소속·실종자 캐시 시드 데이터 준비
  - 담당 Spec: S1-1
...
- [ ] L1-T07 SC-01/02/12 사건 흐름 모의 어댑터와 시드 ID 로더 작성
  ...
  - 예상 작업량: 1d
  - 완료 기준: `spec/harness-scenarios.md`의 seed ID가 fixture로 적재되고, L1이 auth, event, marker seed, OP bootstrap, purge hook mock으로 실행된다.
...
- [ ] L1-T01 배정 사건 가져오기 API 구현
  ...
  - 시나리오: SC-01
  - 관련 FR: FR-01
  - 구현 산출물: `POST /incidents/import-from-seed`, incident/membership/missing_person_cache write path, `INCIDENT_CREATED` publish request, OP1 bootstrap handoff test
  - 예상 작업량: 2d
  - 완료 기준: `POST /incidents/import-from-seed`가 중복 row 없이 incident, missing person cache, membership, commander candidate, `INCIDENT_CREATED`, OP1 bootstrap 요청을 생성한다.

- [ ] L1-T03 실종자 운영 캐시와 소비 계약 구현
  - 담당 Spec: S1-1
  - 필수 참조: `spec/specs/S1-1.json`, `spec/specs/S7.json`, `spec/boundaries.md §6 FR-21`, `spec/boundaries.md §6 FR-22`
  - 연관 Spec: S7, S3-2
  - 시나리오: SC-01, SC-03
  - 관련 FR: FR-21, FR-22
  - 구현 산출물: missing_person_cache entity/DTO, allowlist contract test, 금지 필드 노출 negative test, offline package input fixture validation
  - 예상 작업량: 1d
...
- [ ] L1-T02 사건 상태 전이 보호 규칙 구현
  ...
  - 시나리오: SC-01, SC-12
  - 관련 FR: FR-01, FR-22
  - 구현 산출물: `BOOTSTRAPPING`/`OPEN`/terminal guard policy, `incident_bootstrapping`/closed error tests, consumer fixture states
  - 예상 작업량: 1d
  - 완료 기준: `OPEN` 전 write는 `incident_bootstrapping`, 종료 후 write는 closed/terminal guard로 실패하며, 소비 Lane fixture가 두 상태를 모두 검증할 수 있다.

- [ ] L1-T04 인계 후 기존 기록 조회와 지원 부대 소속 추가 구현
  - 담당 Spec: S1-1
  - 필수 참조: `spec/specs/S1-1.json`, `spec/harness-scenarios.md §1.1`, `spec/boundaries.md §10 SC-02`
  - 연관 Spec: S1-2, S3-1, S5, S8, S3-2
  - 시나리오: SC-02
  - 관련 FR: FR-01
  - 구현 산출물: membership update command, commander assignment 처리, `INCIDENT_MEMBERSHIP_CHANGED` PublishRequest contract test, handover membership fixture, OP1 record continuity test
  - 예상 작업량: 1d
  - 완료 기준: 지구대/파출소 OP1 기록이 계속 조회되고, 기존 row를 revoke하지 않은 채 지원 부대 membership이 추가되며, `INCIDENT_MEMBERSHIP_CHANGED`가 안정적인 id/status/version을 포함한다.
...
- [ ] L1-T06 사건 종료 명령과 데이터 삭제 인계 구현
  ...
  - 시나리오: SC-12
  - 관련 FR: FR-22
  - 구현 산출물: incident close command, `INCIDENT_CLOSED` publish request, active missing_person_cache hard delete, purge handoff contract test
  - 예상 작업량: 2d
  - 완료 기준: close가 `INCIDENT_CLOSED`를 발행하고 신규 write를 차단하며 active missing person cache를 삭제하고 S3-2용 sanitized terminal state를 제공한다.
...
- [ ] L1-I01 SC-01 사건 시작 흐름 통합 검증
  ...
  - 구현 산출물: SC-01 integration test, mock-to-real contract 교체 evidence, OP1 bootstrap convergence check
  - 예상 작업량: 1d
  - 완료 기준: 배정 사건 import, OP1 bootstrap, `OPEN` 전이가 SC-01 e2e red test를 통과한다.
...
- [ ] L1-I02 SC-02 인계·지원 배정 통합 검증
  ...
  - 구현 산출물: SC-02 integration test, support unit assignment fixture, `INCIDENT_MEMBERSHIP_CHANGED` fanout/SSE evidence, FCM recipient evidence, board slot convergence evidence
  - 예상 작업량: 2d
  - 완료 기준: 실종팀 인계와 지원 부대 배정 후에도 OP1 기록이 보존되고, 배정 단말 FCM evidence가 남으며, S4 fanout/SSE와 S3-2 board slot snapshot이 수렴한다.

- [ ] L1-I03 SC-12 사건 종료·캐시 파기 통합 검증
  - 담당 Spec: S1-1
  ...
  - 구현 산출물: SC-12 integration test, close/write-block/purge-start evidence, `INCIDENT_CLOSED`/`INCIDENT_PURGED` outbox/SSE evidence, sanitized terminal board snapshot evidence
  - 예상 작업량: 2d
  - 완료 기준: close가 write를 차단하고 active PII 노출을 제거하며 purge orchestration을 시작하고, `INCIDENT_CLOSED`/`INCIDENT_PURGED` outbox/SSE payload와 S3-2 incident_terminal snapshot이 같은 terminal/closed 상태로 수렴한다.
...
- [ ] L1-D01B SC-04~06 리허설 실행과 시연 증거 수집
  ...
  - 구현 산출물: SC-04~06 rehearsal result, boundary/area/path/marker fixture evidence, board convergence capture target, follow-up fix task ID
  - 예상 작업량: 1d
  - 완료 기준: 지도 기준 범위·구역 할당, 수색 세션·Device GPS 경로, 현장 마커 생성 흐름을 fixture ID와 함께 리허설하고 pass/fail evidence를 첨부하며, 실패는 별도 follow-up fix task로 연결한다.
...
- [ ] L1-D01D SC-10~12 리허설 실행과 시연 증거 수집
  ...
  - 구현 산출물: SC-10~12 rehearsal result, OP/handover/AI/terminal fixture evidence, sanitized terminal state capture, follow-up fix task ID
  - 예상 작업량: 1d
  - 완료 기준: 구역 완료·새 OP 열기, 인수인계·OP 비교·AI 요약, 사건 종료·캐시 파기 흐름을 fixture ID와 함께 리허설하고 pass/fail evidence를 첨부하며, 실패는 별도 follow-up fix task로 연결한다.

## 담당하지 않음

- Role/channel policy implementation: L2/S1-2
- Event outbox/SSE dispatch implementation: L2/S4
- Marker domain writes: L5/S5
- OP lifecycle implementation after bootstrap request: L3/S8
- Package/cache purge hook internals: L2/S1-3, L4/S6, L6/S7
