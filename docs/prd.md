...
| 지구대/파출소 | 팀장 또는 당직자 (현장 지휘관) | 폴리폰 앱 + 웹 상황판 | **지구대/파출소 지휘 계정** | 앱에서는 교대 인수인계를 확인하고, 웹 상황판에서는 이전 OP 수색 이력 확인·새 OP 생성·의심 구역 재확인 지시·실종팀 인계용 수색 이력 정리를 수행 |
...
- 수색 이력 자동 요약 (OP/근무 구간/경로/마커/메모 기반 요약. `DUTY_SHIFT` 범위는 Android 현장 앱의 인수인계 요약, `OP` 범위는 Web 상황판의 OP 정보 요약으로 표시하며, 누락 판정·다음 구역 지시는 금지. Android 하단 네비게이션에는 OP 요약/OP 비교 전용 진입점을 두지 않는다)
...
| SearchHistorySummary | OP 또는 근무 구간의 수색 기록 요약. `DUTY_SHIFT` 범위는 Android 현장 앱의 인수인계 요약, `OP` 범위는 Web 상황판의 OP 정보 요약으로 표시한다. Android에서는 OP 요약/OP 비교를 최상위 화면으로 분리하지 않는다. 누락 판단이나 다음 구역 지시가 아니라 기록을 짧게 정리하는 AI 요약이며, 생성 실패 시 대체 요약 문장을 저장하지 않는다 |
...
| FR-36 | 담당 구역을 배정받은 현장 단말이 해당 TEAM 수색구역 경계 밖 위치로 표시되면 현장에서 즉시 확인할 수 있어야 한다. | 판정 기준은 현재 OP에서 해당 PolicePhone 계정에 ACTIVE 배정된 `search_area.area_level = TEAM` polygon이다. 전체 수색구역 밖 좌표를 structural failure로 거부하는 정책과 구분한다. 앱은 서버 연결 여부와 무관하게 로컬 polygon 포함 여부를 먼저 판단해 짧은 진동과 인앱 안내/OS 알림을 표시한다. 이 안내는 위반·징계성 알림이 아니라 GPS 기준 위치와 담당 구역을 함께 확인하라는 현장 확인용이며, 사용자 문구는 `담당 구역 경계 확인`처럼 중립적으로 표현한다. 네트워크가 가능하면 위치 확인 이벤트를 서버에 저장하고, 서버는 같은 eventId 기준의 FCM data message를 대상 업무폰에 전달할 수 있다. 지휘 상황판에는 긴급 알람이 아니라 운영 참고 이벤트로 노출한다. 반복 알림은 같은 경계 밖 표시 상태에서 cooldown 또는 재진입 후 재표시 기준으로 제한한다. 이 기능은 자동 누락 구역 판단이나 다음 수색 구역 추천으로 확장하지 않는다. |
...
| FR-39 | 시스템은 OP 종료 또는 근무 교대 시점에 기록된 OP·근무 구간·경로·마커·구역 상태·인수인계 메모를 바탕으로 이전 근무자가 어디 일대를 수색했는지 짧게 요약해 보여준다. (MVP2 보고서 AI를 대체하는 P0 MVP) | 수색 이력 요약 입력 데이터는 Suri-Map에 기록된 OP·근무 구간·경로·마커·구역 상태·계정이 직접 작성한 메모로 제한한다. 요약 생성 모델은 수색 누락 여부를 확정하거나 다음 수색 구역을 지시하지 않는다. 요약은 OP 또는 근무 구간별로 `수색 시간 / 수색한 일대 / 차량·도보 구간 / 주요 마커·메모` 중심으로 짧게 표시한다. 요약 생성은 근무 교대 종료 또는 OP 전환 write가 성공한 뒤 서버가 내부 job으로 처리하며, 앱과 웹은 요약 생성 API를 직접 호출하지 않는다. 근무 교대 종료 시 단말 미전송 기록이 남아 있으면 요약은 최종으로 취급하지 않고 동기화 안정화 후 갱신되어야 한다. 요약 생성 job/provider 실패 또는 응답 품질 미달 시에는 `generation_status = FAILED`로 남기고 대체 요약 문장을 저장하지 않는다. 앱과 웹 UI는 실패 안내와 원본 경로·마커·메모 확인을 제공하되, 재생성은 사용자 CTA가 아니라 서버 관리 재시도/재큐잉 정책으로 처리한다. Android 앱은 근무 교대/인수인계 화면에서 `DUTY_SHIFT` 범위 요약을 확인하고, Web 상황판은 OP 상세 또는 수색 이력 화면에서 `OP` 범위 요약과 OP 비교를 확인한다. Android 하단 네비게이션에는 OP 요약/OP 비교 전용 진입점을 두지 않는다. 요약 화면에서는 원본 경로·마커·메모를 함께 확인할 수 있다. |
