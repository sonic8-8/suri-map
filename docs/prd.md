...
- 수색 이력 자동 요약 (OP/근무 구간/경로/마커/메모 기반 요약. `DUTY_SHIFT` 범위는 인수인계 요약, `OP` 범위는 OP 정보 요약으로 표시하며, 누락 판정·다음 구역 지시는 금지)
...
| SearchHistorySummary | OP 또는 근무 구간의 수색 기록 요약. `DUTY_SHIFT` 범위는 인수인계 요약, `OP` 범위는 OP 정보 요약으로 표시한다. 누락 판단이나 다음 구역 지시가 아니라 기록을 짧게 정리하는 AI 요약이며, 생성 실패 시 대체 요약 문장을 저장하지 않는다 |
