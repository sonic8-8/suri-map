...
| OP(Operation Period, 수색 차수) | 사건 내 수색 차수 단위 히스토리 레이어. 사건 가져오기 시 OP1 자동 생성, OP2 이후는 권한 보유자가 수동 생성 |
| 실종팀 | 실종 사건 담당 수색 조직. 사건을 가져오고·종료하며 지원 부대 배정을 수행한다 |
| 지원 부서 | 실종팀의 지원 부대 배정으로 사건에 투입되는 부대(기동대 등). 수색 업무는 실종팀과 동등하게 수행 |
| 지구대/파출소 | 신고 초동 대응 및 1차 수색을 담당하는 일선 조직. 실종 사건이 하루 이상 길어지거나 자살 의심이 있을 때 실종팀으로 인계. 본 시스템의 주요 사용자 |
| 현장 지휘관 | 사건의 지도 기준 범위·구역 분할·완료·차수 관리 권한을 가진 **역할**. 사건당 N명 지정 가능하며 **실종팀 간부, 기동대장, 제대장, 지구대/파출소 팀장 또는 당직자** 등이 시드 기반 자동 부여 + 실종팀 수동 조정으로 결정됨 |
| 간부 / 대원 | 시드 데이터의 직급 필드 기반 구분. 간부는 현장 지휘관 역할 자동 후보이며, 실종팀 간부는 사건 종료·부대 배정 등 실종 담당 권한을 가진다 |
| 부대(Unit) | 기동대 등 경찰 조직 단위. 사건에 다중 배정 가능 |
| 팀(Team) | 부대 하위 조직. 팀 업무폰 N대(현장 인터뷰 기준 6인 1팀당 1대 수준)를 교대 사용 |
...
| 인수인계 메모 | OP·경로·구역 단위로 남기는 정성적 메모(예: "외곽 농로 큰길만 확인", "CCTV 없음"). 카톡 등 비구조 공유를 대체 |
...
- 웹 상황판에서 팀별 경로, 차량/도보 구간, 수색 완료 구역, 재확인 필요 마커, OP별 경로 히스토리 시각화
- OP(수색 차수) 기반 수색 히스토리 레이어로 인수인계와 재수색 판단 보조
- 통신 불안정을 전제로 한 오프라인 저장 + 복구 동기화
- 무전 및 112 공식 시스템 대체가 아닌 판단 보조 도구. 112/실종프로파일링은 mock·seed로 시연하며 직접 연동하지 않음
...
**소속(Affiliation) × 직급(Rank) 2축으로 권한을 결정**한다. 실종팀과 지원 부서(기동대 등)는 상하 관계가 아닌 동등한 수색 조직이며, 차이는 **실종팀만 사건을 종료하고 지원 부대를 배정한다**는 점이다. 지구대/파출소는 초동 대응과 1차 수색을 담당하며, 사건이 길어지거나 자살 의심이 있을 때 실종팀으로 인계한다 (2026-04-24 인터뷰 근거).

| 소속 | 직급 | 주 디바이스 | 계정 단위 | 주요 행동 |
|---|---|---|---|---|
| 실종팀 | 간부 | 폴리폰 앱 + 웹 상황판 | **실종팀 지휘 계정** | 현장 수색 + 사건 가져오기·종료·지원 부대 배정·현장 지휘관 지정, 지도 기준 범위·구역 분할·완료·차수 관리 |
...
| 지구대/파출소 | 경찰관 | 폴리폰 앱 (팀 업무폰·순찰차 업무폰) | **팀 계정 또는 순찰차 계정** | 초동 대응·1차 수색, 순찰차/도보 수색 경로 기록, 단서·재확인 마커 생성, 인수인계 메모 작성 |
...
- 지정 방식: 시드 데이터의 직책(기동대장·제대장·지구대 팀장 등) 기반 **자동 부여** + 실종팀이 필요 시 **추가·교체**
- 실종팀 단독 수색 중에는 실종팀 간부가 현장 지휘관 역할을 겸임
- **지원 부대 배정 이후에도 실종팀 간부의 현장 지휘 권한은 그대로 유지**된다. 기동대장·제대장 합류 시 현장 지휘관이 추가되는 것이지 권한이 이양되는 것이 아니다
...
- 112/실종프로파일링 mock·seed 배정 사건 가져오기, 지구대/파출소 초동 대응, 지원 부대 배정, 사건별 역할 기반 접근 제어
- 팀 업무폰/순찰차 업무폰 GPS 경로 자동 기록, 차량/도보 구간 자동 분리와 상황판 수동 보정, 순찰차 경로 레이어
- OP 기반 수색 히스토리·인수인계 뷰, 수색 세션 시작/일시정지/재개/종료, OP/경로/구역 단위 인수인계 메모
...
- AI 수색 이력 자동 요약 (OP/경로/마커/메모 기반 요약, 누락 판정·다음 구역 지시 금지)
...
> 주: 기존 AI 보고서 초안과 과거 사례/RAG 기반 위험 구역 조회는 현장 니즈·데이터 확보·보안 리스크 대비 MVP 가치가 낮아 제외한다. 진입점/이탈로/차량 접근 가능 지점 레이어는 데이터셋 확보 가능성을 먼저 확인한 뒤 후속 범위로 판단한다. AI 기능은 P0 MVP의 `AI 수색 이력 자동 요약`으로 한정한다.
...
- 수색 보고서 AI 초안 생성 (인터뷰 결과 니즈 약함, AI는 수색 이력 요약으로만 한정)
...
- 부대(Unit)/팀(Team) 편제 관리 UI (Unit·Team 모두 시드 데이터로 운영, §6.5 참조)
...
| 2 | 지원 부대 배정 | 실종팀 간부 | 지원 투입 부대 선택 | 배정된 부대 소속 계정과 현장 지휘관 역할 보유 계정이 사건 접근 권한 획득 |
...
| 8 | 지원 요청 마커 생성 | 사건 배정 계정 (앱) | 진입 불가, 추가 인력, 드론·경찰견 등 무전·공식 채널로 전달한 지원 요청 위치를 마커로 기록 | 실종팀·현장 지휘관에게 상황판·단말 모두에 알림 도달 |
...
| 11 | 인수인계 / 다음 투입 구역 판단 | 현장 지휘관 | 상황판에서 OP별 업무폰·순찰차 경로, 완료 처리된 구역, 재확인 필요 마커, 인수인계 메모, **AI 수색 이력 자동 요약**을 확인한 뒤 필요 시 새 OP를 열고 다음 투입 구역 판단 | 이전 OP와 현재 OP를 겹쳐 보고, AI 요약으로 “어디 일대를 수색했는지”를 확인하며, 새 OP 생성 시 사유(재수색·근무 교대·새 구역 배정·기타)가 기록됨 |
...
2. 실종팀 단독 수색 → 범위가 넓거나 시간이 지체되면 지원 부서(기동대 등) 요청
3. 현장 수색 인원 → 현장 지휘관(실종팀 간부·기동대장·제대장·지구대/파출소 팀장 또는 당직자 등) 간 무전 보고 → 실종팀이 112 시스템에 자유 텍스트로 수색 완료 지역 입력

Suri-Map은 3단계의 **공용 상황판·구조화된 현황**을 보완한다. 1~2와 무전 흐름은 유지하되, MVP/시연에서는 실제 경찰 시스템 직접 연동 대신 112/실종프로파일링 mock·seed 배정 사건을 가져와 초기 입력을 줄인다.
...
| 객체 | 설명 |
|---|---|
| Incident (사건) | 실종 신고 1건 단위. 실종자 정보, 배정 부대/지휘관/지구대·파출소, 지도 기준 범위(map_boundary), 상태 포함. MVP/시연에서는 112/실종프로파일링 mock·seed로 가져오며 사용자가 직접 생성하지 않는다 |
| MissingPerson | 실종자 기본 정보 (사건 메타데이터) |
| Unit (부대) | 기동대 등 상위 조직. DB 시드로 선등록. 지구대/파출소도 동일하게 시드 등록 |
| Team | 팀. Unit에 소속. 복수 대원과 복수 폴리폰을 포함할 수 있는 운영 단위. 인터뷰 기준 6인 1팀당 팀 업무폰 1대 운용 |
| SearchArea (수색 구역) | 지도상 폴리곤. 담당 팀, 수색 상태, 수색 횟수 포함 |
| SearchPath (수색 경로) | **팀 업무폰 또는 순찰차 업무폰 기준 GPS LineString**. 시스템이 GPS 속도 기반으로 차량 구간(`vehicle`)과 도보 구간(`foot`)을 자동 분리하며, 상황판에서 구간 단위 수동 보정 가능. 계정은 경로 주체가 아니라 해당 시점의 팀/순찰차/지휘 운영 주체로 기록된다. 상황판에서는 소속(실종팀/지원 부서/지구대·파출소)·구간 유형·OP·현장 지휘관 역할 여부에 따라 시각적으로 구분된다 |
| SearchSession (수색 세션) | 팀 업무폰 또는 순찰차 업무폰이 `수색 시작`부터 `수색 종료`까지 만든 1회 기록 단위. OP에 귀속되며 OP 하위에 N개 가능. `진행 중 / 일시정지 / 종료` 상태와 시작·종료 시각, 조작 계정(팀 계정·순찰차 계정·지휘 계정)을 보유. 일시정지 구간은 지도에서 경로 선으로 이어붙이지 않는다 |
| OperationPeriod (OP, 수색 차수) | 사건 내 수색 차수 단위 히스토리 레이어. 경로·마커·완료 구역·인수인계 메모를 묶는다. 사건 가져오기 시 `OP1`이 자동 생성되고, `OP2~`는 권한 보유자(현장 지휘관·실종팀 간부·지구대/파출소 팀장 또는 당직자)가 수동 생성. 사유 enum: `재수색 / 근무 교대 / 새 구역 배정 / 기타`. 이전 OP와 현재 OP를 겹쳐 볼 수 있다 |
| Marker (마커) | 단서, 실종자 발견, 지형 상태, 지원 요청, **재확인 필요**, **수색 제외/확인 완료** 등 운영 유형 Point. 타입별 레이어 분리. 마커는 OP에 귀속. 작성 주체는 로그인한 팀 계정·순찰차 계정·지휘 계정 기준으로 기록 |
| HandoverMemo (인수인계 메모) | OP·경로·구역 단위에 남기는 정성적 메모. 작성 계정, 작성 시각, 연결 OP/대상 객체 ID 보유. 카톡 등 비구조 공유를 대체하는 구조화 기록 |
| Account | 팀 계정·순찰차 계정·지휘 계정. 속성: 소속(실종팀·지원 부서·지구대/파출소), 계정 유형, 연결 Team/Device/역할, 현장 지휘관 역할 여부. **개인이 아니라 운영 단위**로 기록된다 |
| Device (단말) | 폴리폰 단말 정보(팀 업무폰/순찰차 업무폰 구분 포함)와 오프라인 지도/동기화 상태를 추적하는 객체 |
| Assignment | 사건에 부대·팀·지휘관·지구대/파출소 배정 관계 및 역할 |
| SyncEvent | 동기화 이벤트 (idempotency key 포함) |
| LocationAccessLog | 위치정보 접근 기록 (위치정보법 요구사항) |

**관계 요약**
- `Team` belongs to `Unit`
- `Team` has many `Accounts`
- `Incident` has many assigned `Units` (다중) + **many assigned `FieldCommander Accounts`** (사건당 N명) + many assigned `Teams` + many assigned `지구대/파출소`
- `Incident` has many `OperationPeriods` (OP1 자동 생성, OP2~ 수동)
- `OperationPeriod` has many `SearchSessions` / `Markers` / `SearchAreas` / `HandoverMemos`
- `SearchPath`의 주체는 `Device`(팀 업무폰 또는 순찰차 업무폰)이며, 같은 경로 안에서 구간(`vehicle`/`foot`)이 자동 분리되고 OP·세션 단위로 묶인다
- `Account`는 개인이 아니라 팀·순찰차·지휘 역할 단위의 조작자·메모 작성자·인수인계 확인자로 주요 조작 이력에 기록된다
- 현장 지휘관 역할은 실종팀 간부·기동대장·제대장·지구대/파출소 팀장 또는 당직자 등 **직책 보유자에게 시드 기반 자동 부여**되며, 실종팀이 추가·교체할 수 있다
- 배정된 `Accounts`는 계정 유형 또는 소속 팀/부대/지구대를 통해 사건 접근 권한을 획득
- 실종팀 지휘 계정은 동시에 여러 `Incident`에 배정 가능하며, 현장 팀/순찰차 계정은 배정된 사건 범위에서만 접근한다
...
- **운영 마커 레이어**: 재확인 필요 / 수색 제외·확인 완료
- **지형 상태 레이어**: 접근 곤란 지형 표시
- **지원 요청 레이어**: 진입 지원 / 추가 인력 / 드론 지원 요청 기록 / 경찰견 지원 요청 기록 등 무전·공식 채널로 전달한 지원 요청 위치. Suri-Map은 요청 위치를 지도에 기록·공유할 뿐, 출동 요청 전송·승인·장비 배정을 처리하지 않는다
...
- **Unit, Team 모두 DB 시드 데이터로 선등록**한다 (관리 UI 없음, 2차 검토)
- 편제 변경 시 운영 측에서 DB 직접 관리
- Unit 최소 필드: `id`, `이름`, `상위 조직(경찰서/시도경찰청)`
- Team 최소 필드: `id`, `이름`, `소속 Unit`
...
| FR-07 | 생성된 수색 구역의 진행 상태를 시각적으로 파악할 수 있다. | 계정이 생성한 수색 구역 안에서 할당됨/수색중/완료/재수색 필요 상태가 지도에서 구분 표시된다. 접근 곤란은 구역 상태가 아니라 FR-16의 지형 상태 레이어로 표시한다. 시스템이 지도 전체 또는 임의 지역의 확인 누락 여부를 자동 판단하거나 별도 하이라이트하지 않는다. |
| FR-11 | 수색 차수(OP)마다 트랙·어사인먼트·마커·인수인계 메모를 별도로 묶어, 보고 싶은 OP만 선택하여 화면에 나타낼 수 있고 OP 간 비교가 가능하다. (v3 갱신 — OP 비교 보기 추가) | OP를 최신순으로 정렬하여 선택할 수 있게 한다. 원하는 OP를 1개 이상 선택하면 선택된 OP의 정보만 화면에 표시한다. **이전 OP와 현재 OP를 겹쳐 보고 중복 여부와 재확인 필요 지점을 비교할 수 있다.** 사건 가져오기 시 OP1이 자동 생성된다. |
| FR-12 | 현장 지휘관이 구역별 수색 상태와 수색 횟수를 확인하여, 새 OP를 연 후, 필요 시 재확인 구역·마커·메모를 지정하고 수색 인원을 배정한다. (v3 갱신 — OP 사유 enum + 권한 주체 확장) | **현장 지휘관 역할을 가진 계정(실종팀 간부·기동대장·제대장·지구대/파출소 팀장 또는 당직자) 누구나** **웹 상황판에서** `새 OP 열기`를 수행할 수 있다. 실행 시 OP 사유를 `재수색 / 근무 교대 / 새 구역 배정 / 기타` 중 하나로 기록한다. 해당 사건의 OP 카운터가 증가하고, **이전 OP의 트랙·어사인먼트·마커·메모는 보존된 채** 새 OP 레이어가 생성된다. 재확인이 필요한 위치나 범위는 새 구역을 그리거나 재확인 필요 마커·메모로 지정할 수 있으며, 담당 팀과 인원을 배정하면 배정된 계정/단말에 공유된다. OP 이력에는 실행 계정·OP 사유·배정 변경 이력이 기록된다. |
| FR-13 | 구역별 수색 상태와 수색 횟수를 함께 관리/표현한다. | 구역 클릭 시 `상태: 재수색 완료 / 수색 횟수: 2회` 표시. |

### 7.4 마커

| ID | 요구사항 | Acceptance Criteria |
|---|---|---|
| FR-10 | 지도상에 단서·실종자 발견·운영(재확인 필요·수색 제외/확인 완료) 마커를 표시하고 공유한다. (v3 갱신 — 운영 마커 유형 추가) | 사용자 현장 마커 생성은 **폴리폰 앱 전용**이며, 현장 GPS 자동 입력을 전제로 한다. 마커 유형은 단서, 실종자 발견, 지형 상태, 지원 요청, **재확인 필요**, **수색 제외/확인 완료**를 포함한다. 단서 유형에는 사용자가 현장에서 발견한 단서뿐 아니라 mock·seed 사건 정보의 신고자 진술 위치, CCTV 마지막 포착 위치 등 초기 기준점도 포함할 수 있다. 초기 기준점 마커는 사건 가져오기 시 시스템이 생성하고, 웹 상황판에서 권한 보유자가 위치를 조정하거나 추가할 수 있다. 생성·수정 결과는 상황판/다른 단말에 체감상 즉시 반영된다. 실종자 발견 마커는 강조 알림을 발생시킨다. |
...
| FR-15 | 단서/발견 마커에 시간, 보고 주체, 출처, 유형, 메모를 기록한다. 메모는 마커뿐 아니라 OP·경로·구역 단위 인수인계 메모로도 작성할 수 있다 (v3 갱신, 상세는 FR-37). | 앱 마커 생성 흐름은 바텀시트 UI로 유형을 원탭 선택하면 즉시 저장된다. 위치(자동/필요 시 수동 조정), 시간(자동), 보고 주체(자동 = 로그인 계정), 출처(사용자 입력 또는 system/mock·seed)는 기록되며, 메모·사진은 선택 입력이다. 필수 필드는 유형 하나로 최소화하여 현장에서 최소 조작으로 마커를 생성할 수 있어야 한다. **OP·경로·구역 단위 인수인계 메모는 FR-37에서 별도 정의한다.** |
| FR-16 | 접근 곤란 지형을 별도 레이어로 표시한다. | 지형 상태 레이어 on/off 토글 지원. 마커 생성은 앱 전용. *(용어 기준: SDD 단계에서 현업 확인)* |
| FR-17 | 진입 지원, 추가 인력, 드론·경찰견 등 현장 지원 요청 위치를 별도 레이어로 표시한다. (v3 갱신 — 요청 전송·승인이 아니라 위치 기록 마커) | 지원 요청 마커는 앱에서 생성하며, 생성 시 실종팀·현장 지휘관에게 알림을 발송한다. 마커에는 위치, 작성자, 작성 시각, 요청 유형, 메모를 기록한다. 요청 유형 예시는 진입 지원, 추가 인력, 드론 지원 요청 기록, 경찰견 지원 요청 기록 등으로 둘 수 있다. 이 마커는 무전·공식 채널로 전달한 요청 위치를 지도에 남기는 기록이며, Suri-Map은 실제 출동 요청 전송·승인·장비 배정·장비 연동을 처리하지 않는다. 실제 투입 판단과 요청은 기존 지휘·무전·공식 채널을 따른다. 드론·경찰견 경로와 확인 구역 표시는 이번 구현 범위에서 제외한다. |
...
| FR-23 | 시스템은 자동으로 확인 누락 구역을 판단하지 않고, OP별 경로 히스토리와 재확인 필요 마커를 통해 현장 지휘관의 판단을 보조한다. (v3 갱신 — 사각지대 자동 하이라이트 제외) | 상황판은 OP별 업무폰·순찰차 경로, 완료 처리된 구역, 재확인 필요 마커를 함께 표시한다. 현장 지휘관은 이전 OP와 현재 OP를 겹쳐 보고, 사람이 직접 재확인 가능 지점을 판단한다. 시스템은 확인 누락 구역을 자동 확정하거나 다음 수색 구역을 추천하지 않는다. |
...
| ID | 요구사항 | Acceptance Criteria |
|---|---|---|
| FR-32 | OP(수색 차수) 단위로 경로·마커·완료 구역·인수인계 메모를 묶어 인수인계 히스토리 레이어를 제공한다. (FR-11/FR-12 강화) | 사건을 처음 가져오면 사건 섹션과 `OP 1차`가 자동 생성된다. `OP 2차` 이후는 권한 보유자(실종팀 간부·기동대장/제대장·지구대/파출소 팀장 또는 당직자)가 수동 생성하며, 생성 시 사유를 `재수색 / 근무 교대 / 새 구역 배정 / 기타` 중 하나로 기록한다. 사용자는 OP별 경로·마커·완료 구역을 선택적으로 표시할 수 있고, 이전 OP와 현재 OP를 겹쳐 볼 수 있다. OP에는 수색 메모와 재확인 필요 메모를 남길 수 있다. 근무 교대·인수인계 시 현재 OP의 기록을 확인하거나 새 OP를 열어 이어갈 수 있다. |
| FR-33 | 시스템은 개인 사용자가 아니라 팀 업무폰 또는 순찰차 업무폰 기준으로 수색 경로를 기록하며, GPS 속도 기반으로 차량 구간과 도보 구간을 자동 분리한다. (FR-02/FR-04/FR-25 전제 통합) | 경로의 주체는 팀 업무폰 또는 순찰차 업무폰 중 하나가 될 수 있다. 계정은 개인이 아니라 팀·순찰차·지휘 운영 단위의 조작자·메모 작성자·인수인계 확인자로 기록된다. 기동대처럼 팀당 업무폰 1대를 사용하는 경우 팀 업무폰 경로로 표시한다. 전날 당직자가 사용하던 팀 업무폰 또는 순찰차 업무폰을 다음 근무자가 이어 사용하는 경우에도 같은 업무폰 경로가 OP·세션 단위로 구분된다. MVP 자동 분리는 GPS 속도 기준으로 차량/도보 구간을 추정하되, 짧은 GPS 튐을 막기 위해 일정 시간 이상 지속된 속도 변화만 반영한다. 자동 분류가 틀린 경우 **상황판(웹)** 에서 구간 단위로 차량/도보 유형을 보정할 수 있다. 보정 UI는 MVP에 포함한다. 상황판에서는 업무폰 경로·차량 구간·도보 구간을 시각적으로 구분한다. |
| FR-34 | 시스템은 수색 경로 기록을 상시 위치 추적이 아니라 사건·OP에 연결된 수색 세션 단위로 관리하며, 시작/일시정지/재개/종료 상태를 명시적으로 다룬다. | 업무폰을 운용 중인 현장 계정이 `수색 시작`을 누르면 현재 사건과 현재 OP에 연결된 수색 세션이 생성되고 경로 기록이 시작된다. `일시정지`는 같은 세션 안에서 위치 기록을 임시 중단하며, `재개`는 같은 세션에서 기록을 이어간다. `수색 종료`는 해당 세션을 마감하고 OP 히스토리에 반영한다. 일시정지된 구간은 지도에서 경로 선으로 이어붙이지 않는다. 기록 중 상태는 화면에서 명확히 표시한다. |
...
| FR-37 | 시스템은 수색 경로·구역·OP에 대한 정성적 인수인계 메모를 남길 수 있다. (FR-15 마커 메모 확장) | 사용자는 경로·구역·OP 단위로 메모를 작성할 수 있다. 메모에는 작성 계정(팀 계정·순찰차 계정·지휘 계정), 작성 시각, 연결된 OP가 기록된다. 상황판에서 인수인계 메모를 시간순 또는 OP별로 확인할 수 있다. 카톡 등 비구조 공유를 대체하는 구조화 기록으로 사용한다. |
| FR-39 | 시스템은 OP 종료 또는 근무 교대 시점에 기록된 OP·경로·마커·구역 상태·인수인계 메모를 바탕으로 이전 근무자가 어디 일대를 수색했는지 짧게 요약해 보여준다. (MVP2 보고서 AI를 대체하는 P0 MVP) | AI 요약 입력 데이터는 Suri-Map에 기록된 OP·경로·마커·구역 상태·계정이 직접 작성한 메모로 제한한다. AI는 수색 누락 여부를 확정하거나 다음 수색 구역을 지시하지 않는다. 요약은 OP 또는 세션별로 `수색 시간 / 수색한 일대 / 차량·도보 구간 / 주요 마커·메모` 중심으로 짧게 표시한다. AI 호출 실패 또는 응답 품질 미달 시에는 동일 입력 데이터 기반 템플릿 요약으로 대체한다. 요약은 근무 교대 또는 OP 전환 화면에서 기본 표시하고, 상황판의 OP 상세 화면에서 다시 확인할 수 있어야 한다. 요약 화면에서는 원본 경로·마커·메모를 함께 확인할 수 있다. |
...
| 수색 세션 시작/일시정지/재개/종료 | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) |
| 현장 마커 생성 (단서/발견/지형/지원 요청/재확인 필요/수색 제외·확인 완료) | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) | ✅ (앱) |
| 초기 기준점 마커 추가·수정 (마지막 목격/CCTV/신고자 진술 위치 등) | ✅ (웹) | ❌ | ✅ (웹) | ❌ | ✅ (웹) | ❌ |
| 마커 수정·삭제 | ✅ (모든 마커) | ✅ (자기 계정 생성분만) | ✅ (모든 마커) | ✅ (자기 계정 생성분만) | ✅ (모든 마커) | ✅ (자기 계정 생성분만) |
| 인수인계 메모 작성 (OP/경로/구역 단위) | ✅ (웹·앱) | ✅ (웹·앱) | ✅ (웹·앱) | ✅ (웹·앱) | ✅ (웹·앱) | ✅ (웹·앱) |
| GPS 경로 수정·삭제 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |

**주:**
- 사건 가져오기/활성화·종료·지원 부대 배정·실종자 정보 입력은 **실종팀 간부 중심**으로 둔다. 다만 지구대/파출소 팀장·당직자는 초동 대응 단계의 사건 가져오기에 한해 권한을 가진다 (인터뷰 근거). 실종팀 대원은 배정 사건 조회와 정보 보완만 수행한다.
- 지도 기준 범위·구역·차수 등 **현장 지휘 업무는 실종팀 간부, 지원 부서 간부, 지구대/파출소 팀장·당직자가 동등**
- **OP 생성 권한**은 현장 지휘관 + 지구대/파출소 팀장·당직자가 공유. 사유 enum(`재수색 / 근무 교대 / 새 구역 배정 / 기타`) 기록 필수 (인터뷰 근거)
...
Suri-Map은 **운영 보조 시스템**이며, 공식 사건 기록과 실종자 개인정보의 **장기 보존 원본은 112 계열 시스템**이다. 따라서 본 시스템은 운영 중 캐시로 실종자 정보를 보유하고, 사건 종료 시 Suri-Map에서는 제거한다. 장기 보존 책임(예규 5/10년 등)은 원본 시스템이 진다.
...
| 실종자 개인정보/사진 (운영 캐시) | 사건 종료 시 Suri-Map active DB·단말에서 즉시 제거 | 장기 보존 원본은 112 계열 시스템 — 실종아동등 업무처리 규칙 상 5/10년 보존 책임은 원본 시스템이 담당 |
...
- [ ] **112 계열 시스템이 실종자 개인정보/사진의 장기 보존(예규 5/10년) 원본 역할을 실제로 담당하는지 확정** — Suri-Map이 운영 캐시로만 보유하는 전제의 법적 타당성
