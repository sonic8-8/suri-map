 ...
 *
 * <p>maparea/fixturetest와 같은 역할로, fixture 값이 문서/계약과 어긋나지 않는지 확인한다.</p>
 */
...
class OperationalPeriodFixtureExactnessTest {
    ...
    void 공통_op_fixture_값이_고정되어_있다() {
        ...
        assertThat(OperationalPeriodFixtures.OP_REASON_PERSISTED).containsExactly(
                "BOOTSTRAP",
                "SHIFT_CHANGE",
                "RE_SEARCH",
                "NEW_AREA",
        ...
        assertThat(OperationalPeriodFixtures.OP_REASON_MANUAL_CREATE).containsExactly(
                "SHIFT_CHANGE",
                "RE_SEARCH",
                "NEW_AREA",
                "OTHER"
    ...
    void guard_consumer_transition_idempotency_fixture_값이_고정되어_있다() {
        ...
        assertThat(CurrentOpConsumerFixtures.CURRENT_OP_CONSUMERS).containsExactly(
                "S7 offline manifest builder",
                "S3-1 search session/path write",
                "S5 marker/support/person write"
        );
        assertThat(CurrentOpConsumerFixtures.FORBIDDEN_S8_OWNER_WRITES).containsExactly(
        ...
        assertThat(OpTransitionFixtures.SC10_BOARD_PROBE_SLOTS).containsExactly(
                "op_toggle",
                "op_history",
                "handover_status"
        );
        ...
        assertThat(S8IdempotencyFixtures.IDEMPOTENT_COVERED_ENDPOINTS).containsExactly(
