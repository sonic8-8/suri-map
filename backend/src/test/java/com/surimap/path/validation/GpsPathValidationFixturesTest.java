...
class GpsPathValidationFixturesTest {
    ...
    void structuralFixturesFreezeInvalidGeometryCasesBeforeValidatorImplementation() {
        ...
        assertThat(GpsPathValidationFixtures.BATCH_LIMIT_EXCEEDED.points())
                .isSortedAccordingTo(Comparator.comparing(GpsPathValidationFixtures.GpsPointFixture::clientTs));
        assertThat(GpsPathValidationFixtures.BATCH_LIMIT_EXCEEDED.points())
                .allSatisfy(point -> {
                    assertThat(point.lon()).isBetween(
                    ...
                    assertThat(point.lat()).isBetween(
                    ...
                    assertThat(point.lon().scale()).isLessThanOrEqualTo(
                            GpsPathValidationCriteria.CANONICAL_COORDINATE_SCALE
                    );
                    assertThat(point.lat().scale()).isLessThanOrEqualTo(
                            GpsPathValidationCriteria.CANONICAL_COORDINATE_SCALE
                    );
                    assertThat(point.speedMps()).isBetween(
                            BigDecimal.ZERO,
    ...
    void qualityFixturesFreezeExcludedPointCriteriaBeforeValidatorImplementation() {
        ...
        assertThat(GpsPathValidationFixtures.QUALITY_FAILURE_FIXTURES)
                .allSatisfy(fixture -> assertThat(fixture.expectedDisposition()).isEqualTo("excludedPoints"));
        assertThat(GpsPathValidationFixtures.QUALITY_FAILURE_FIXTURES)
                .filteredOn(fixture -> fixture.violationDirection() == GpsPathValidationFixtures.ViolationDirection.BELOW_MIN)
                .allSatisfy(fixture -> assertThat(fixture.measuredValue()).isLessThan(fixture.threshold()));
        assertThat(GpsPathValidationFixtures.QUALITY_FAILURE_FIXTURES)
                .filteredOn(fixture -> fixture.violationDirection() == GpsPathValidationFixtures.ViolationDirection.ABOVE_MAX)
    ...
    void timestampSkewFixtureFreezesDerivableClockSkewInputs() {
        ...
        assertThat(fixture.points()).singleElement().satisfies(point -> {
            var skewSeconds = Math.abs(Duration.between(fixture.serverReceivedAt(), point.clientTs()).getSeconds());

            assertThat(BigDecimal.valueOf(skewSeconds)).isEqualByComparingTo(fixture.measuredValue());
        });
        assertThat(fixture.expectedReason()).isEqualTo(GpsPathValidationFixtures.QualityFailureReason.CLOCK_SKEW);
    }
    ...
    void distanceJumpFixtureFreezesDerivableFiveSecondSamplePair() {
        ...
        assertThat(Duration.between(fixture.points().get(0).clientTs(), fixture.points().get(1).clientTs()))
        ...
        assertThat(BigDecimal.valueOf(Math.round(derivedDistanceMeters))).isEqualByComparingTo(fixture.measuredValue());
        assertThat(fixture.expectedReason()).isEqualTo(GpsPathValidationFixtures.QualityFailureReason.DISTANCE_JUMP);
    }
    ...
    private static double distanceMeters(
        ...
        double haversine = Math.sin(deltaLat / 2) * Math.sin(deltaLat / 2)
                + Math.cos(previousLat) * Math.cos(currentLat) * Math.sin(deltaLon / 2) * Math.sin(deltaLon / 2);
