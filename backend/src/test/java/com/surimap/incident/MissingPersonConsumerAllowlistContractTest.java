...
class MissingPersonConsumerAllowlistContractTest {
  ...
  private static final List<String> DETAIL_MISSING_PERSON_RESPONSE_FIELDS =
      List.of(
          "incidentId",
          "displayName",
          "photoObjectKey",
  ...
  // S1-1이 incident detail과 S7 package 입력에 넘길 수 있는 active missing_person source 필드만 둔다.
  private static final List<String> S1_1_ALLOWED_MISSING_PERSON_SOURCE_FIELDS =
      List.of(
          "incidentId",
          "displayName",
          "photoObjectKey",
  ...
  void incident_detail_missing_person_dto_exposes_only_s1_1_consumer_allowlist() {
    assertAllowlist(DETAIL_MISSING_PERSON_RESPONSE, DETAIL_MISSING_PERSON_RESPONSE_FIELDS);
  }

  @Test
  @DisplayName("S1-1 실종자 소비 projection은 importedAt을 내부 값으로만 둔다")
  void missing_person_consumer_view_keeps_imported_at_internal() {
    assertAllowlist(MISSING_PERSON_CONSUMER_VIEW, S1_1_ALLOWED_MISSING_PERSON_SOURCE_FIELDS);
  }
  ...
  private static void assertAllowlist(String className, List<String> allowedFields) {
    assertThat(recordFields(className))
        .containsExactlyElementsOf(allowedFields)
  ...
  private static BoardSourceRow terminalIncidentRowWithMissingPersonFields() {
    return new BoardSourceRow(
        ...
        Map.ofEntries(
            ...
            Map.entry("photoUrl", "/mock-upload/fixture-photo-object-key"),
