...
class GeometrySpatialMapperIntegrationTest extends PostGisIntegrationTestSupport {
  ...
  @Autowired private GeometrySpatialMapper mapper;

  @Test
  @DisplayName("Polygon 실제 면적이 400m2 이상인지 PostGIS geography 면적으로 검증한다")
  void polygon_면적_검증() {
    assertThat(
            mapper.isAreaAtLeastM2(
                SpatialSqlFixtures.SEARCH_AREA_WKT, MINIMUM_POLYGON_AREA_M2_VALUE))
        .isTrue();

    assertThat(
            mapper.isAreaAtLeastM2(SpatialSqlFixtures.TINY_AREA_WKT, MINIMUM_POLYGON_AREA_M2_VALUE))
        .isFalse();
  }
  ...
  void polygon_cover_검증() {
    assertThat(mapper.covers(SpatialSqlFixtures.BOUNDARY_WKT, SpatialSqlFixtures.SEARCH_AREA_WKT))
        .isTrue();

    assertThat(
            mapper.covers(SpatialSqlFixtures.BOUNDARY_WKT, SpatialSqlFixtures.OUTSIDE_BOUNDARY_WKT))
        .isFalse();
  }
  ...
  void polygon_overlap_by_area_검증() {
    assertThat(
            mapper.overlapsByArea(
                SpatialSqlFixtures.BOUNDARY_WKT, SpatialSqlFixtures.SEARCH_AREA_WKT))
        .isTrue();

    assertThat(
            mapper.overlapsByArea(
                SpatialSqlFixtures.SEARCH_AREA_WKT, SpatialSqlFixtures.TOUCHING_AREA_WKT))
        .isFalse();
  }
}
