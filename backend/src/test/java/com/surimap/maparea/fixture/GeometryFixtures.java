 ...
 *
 * <p>기준 문서: docs/spec/specs/S2.json geometry rules,
 * docs/spec/harness-scenarios.md geometry/GPS fixture.</p>
 */
public final class GeometryFixtures {

    // TODO: S2 geometry value object/constant가 생기면 CRS, GeoJSON type, invalid case 이름 문자열을 해당 타입 기준으로 교체한다.
    ...
    /** SC-04 하네스 좌표가 들어와야 하는 fixture bbox(bounding box).
     * 즉, 테스트 시나리오에서 좌표가 들어와야 하는 사각형 범위
     **/
    ...
    /** validMapBoundaryRing()의 EPSG:4326 bbox: [minLon, minLat, maxLon, maxLat]. */
    public static final List<BigDecimal> BOUNDARY_GEOMETRY_BBOX = List.of(
    ...
    public static final List<BigDecimal> AREA_GEOMETRY_BBOX = List.of(
    ...
    /**
     * 정상 map_boundary Polygon outer ring (harness-scenarios.md §6).
     */
    public static List<List<BigDecimal>> validMapBoundaryRing() {
        return List.of(
    ...
    /**
     * 정상 marker 기준 좌표.
     * <p>S5 owned reference. L3는 boundary/area validation reference 용도로만 참조.</p>
     */
    ...
    /**
     * coord-outside-envelope: bbox 밖 좌표.
     * 기대 동작: invalid_geometry 거부.
     */
    ...
    /**
     * coord-latlon-swapped: lat/lon 순서가 뒤바뀐 좌표.
     * 기대 동작: invalid_geometry 거부.
     */
    ...
    /**
     * precision-over-6dp: 소수점 7자리 좌표.
     * 기대 동작: 6자리로 반올림해 canonical coordinate로 정규화.
     */
    ...
    /**
     * 좌표값이 docs에 고정되지 않은 invalid 케이스 이름 목록.
     * 각 테스트가 적절한 좌표 조합을 자체 생성한다.
     */
    public static final List<String> INVALID_CASES_WITHOUT_FIXED_COORDINATES = List.of(
            "polygon-unclosed",
    ...
    /** 정상 map_boundary GeoJSON Polygon 응답 shape. */
    public static GeoJsonPolygon validMapBoundaryPolygon() {
        return new GeoJsonPolygon("Polygon", List.of(validMapBoundaryRing()));
    }
