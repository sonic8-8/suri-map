 ...
 *
 * mock112.enabled=true, dev profile에서만 활성화된다.
 * Suri-Map이 직접 사건을 생성하지 않고, polling으로 감지만 한다.
 */
@Component
...
public class Mock112PollingJob {
    ...
    public Mock112PollingJob(ExternalIncidentAdapter adapter,
    ...
    public void pollNewIncidents() {
        try {
            ...
            if (!readyIncidents.isEmpty()) {
                ...
                if (added > 0) {
                    log.info("mock 112 새 사건 {}건 감지 (총 후보 {}건)",
                            added, candidateStore.candidateCount());
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            for (String sourceIncidentId : importedIds) {
                ...
                if (incident != null && incident.assignments() != null) {
                    assignmentHandler.handleAssignmentChanges(
                            sourceIncidentId, incident.assignments());
                }
            }
        } catch (Exception e) {
