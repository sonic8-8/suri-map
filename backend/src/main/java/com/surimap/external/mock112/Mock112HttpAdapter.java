 ...
 *
 * <p>mock112.enabled=true 일 때만 활성화된다. MOCK_112_BASE_URL 환경변수로 mock 112 서버 주소를 설정한다.
 */
@Component
...
public class Mock112HttpAdapter implements ExternalIncidentAdapter {
  ...
  public List<ExternalIncident> fetchReadyIncidents() {
    ...
    try {
      ...
      return body.stream().map(this::mapToExternalIncident).toList();
    } catch (Exception e) {
  ...
  public void markImported(String sourceIncidentId) {
    String url =
        properties.getBaseUrl() + "/mock-112/incidents/" + sourceIncidentId + "/mark-imported";
    try {
