 ...
 *
 * <p>이 클래스는 자기 교차, 면적 부족, bbox 이탈, ring 닫힘 오류 같은
 * Polygon의 유효성 문제를 직접 판정하지 않는다.</p>
 * <p>대신 실제 검증 로직이 어떤 기준으로 Polygon을 검사해야 하는지에 필요한
 * SRID, CRS, 좌표 순서, bbox 범위, 좌표 정밀도, 최소 면적 기준을 제공한다.</p>
 *
...
public record GeometryPolicy(
    ...
    public static boolean isValidLongitude(BigDecimal lon) {
        ...
        return lon.compareTo(MIN_VALID_LON) >= 0
                && lon.compareTo(MAX_VALID_LON) <= 0;
    }
    ...
    public static boolean isValidLatitude(BigDecimal lat) {
        ...
        return lat.compareTo(MIN_VALID_LAT) >= 0
                && lat.compareTo(MAX_VALID_LAT) <= 0;
    }

    /**
     * 좌표가 S2 하네스 bbox 내부에 있는지 확인한다.
     *
     * <p>좌표 순서는 [경도, 위도] 기준이다.</p>
     *
     * @param lon 검사할 경도값
     * @param lat 검사할 위도값
     ...
     *
     * <p>이 메서드는 invalid_geometry 여부를 판단하는 핵심 검증 메서드가 아니다.</p>
     *
