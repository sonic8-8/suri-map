...
public interface GeometrySpatialMapper {
  ...
  Boolean isAreaAtLeastM2(
      @Param("polygonWkt") String polygonWkt, @Param("minimumAreaM2") BigDecimal minimumAreaM2);

  /**
   * 자식 Polygon이 부모 Polygon 내부에 완전히 포함되는지 확인한다. 부모 외곽선과 자식 외곽선이 맞닿거나 일부 공유되는 경우도 포함으로 본다.
   *
  ...
  /**
   * 두 Polygon이 면적을 가진 겹침 관계인지 확인한다. 외곽선만 맞닿는 경우는 교집합 면적이 0이므로 겹침으로 보지 않는다.
   *
