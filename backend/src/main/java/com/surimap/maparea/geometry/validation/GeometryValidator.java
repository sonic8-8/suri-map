 ...
 *
 * <p>이 클래스는 DB나 PostGIS 없이 확인할 수 있는 1차 검증만 담당한다.
 *
 * <p>좌표 구조, null 여부, EPSG:4326 경도·위도 범위, S2 하네스 bbox 포함 여부, 소수점 6자리 정규화, 연속 중복 좌표 제거, ring 닫힘 여부를
 * 검사한다.
 *
 * <p>자기 교차, 실제 면적 400㎡ 이상 여부, active overall_search_area 내부 포함 여부, split child 간 겹침 여부는 PostGIS 기반
 * 검증 또는 상위 서비스에서 처리한다.
 */
public class GeometryValidator {
   ...
   *
   * <p>각 좌표를 소수점 6자리로 맞춘 뒤, 연속 중복 좌표를 제거한다.
   *
   * <p>정규화된 ring의 좌표 개수, ring 닫힘 여부, EPSG:4326 범위, S2 하네스 bbox 포함 여부를 검사한다.
   *
   ...
   *
   * <p>정규화된 결과가 필요하지 않고, 입력값의 유효 여부만 확인할 때 사용한다.
   *
   * @param ring GeoJSON Polygon의 outer ring 좌표 목록
   */
   ...
   *
   * <p>좌표는 GeoJSON 기준인 [경도, 위도] 순서여야 한다.
   *
   * <p>경도와 위도를 소수점 6자리로 맞춘 뒤, EPSG:4326 범위와 S2 하네스 bbox 포함 여부를 검사한다.
   *
   ...
   *
   * <p>정규화된 결과가 필요하지 않고, 좌표의 유효 여부만 확인할 때 사용한다.
   *
   * @param point [경도, 위도] 좌표
   */
   ...
   *
   * <p>각 좌표는 EPSG:4326 경도·위도 범위 안에 있어야 하고, S2 하네스 bbox 내부에 포함되어야 한다.
   *
   ...
   *
   * <p>각 좌표는 [경도, 위도] 구조 검증, null 검증, 소수점 6자리 정규화, 범위 검증을 거친다.
   *
   * @param ring Polygon outer ring 좌표 목록
   ...
   * 연속으로 중복된 좌표를 제거한다.
   *
   * <p>중간에 같은 좌표가 연속으로 반복되면 하나만 남긴다.
   *
   * <p>단, 첫 좌표와 마지막 좌표가 같은 ring 닫힘 좌표는 제거하지 않는다.
   *
   * @param ring 정규화된 Polygon outer ring 좌표 목록
   ...
   *
   * <p>경도는 -180 이상 180 이하, 위도는 -90 이상 90 이하이어야 한다.
   *
   * @param lon 경도
   * @param lat 위도
   */
  ...
  private boolean isSamePoint(List<BigDecimal> left, List<BigDecimal> right) {
    ...
    return leftLon.compareTo(rightLon) == 0 && leftLat.compareTo(rightLat) == 0;
  }
}
