...
/**
 * S2 overall_search_area, search_area, split Polygon 검증 흐름을 담당하는 서비스.
 *
 * <p>이 서비스는 도메인 시나리오별로 필요한 검증 순서를 조립한다.
 *
 * <p>좌표 구조, ring 닫힘, bbox, 정규화 같은 기본 검증은 {@link GeometryValidator}에 위임한다.
 *
 * <p>실제 면적, overall_search_area 내부 포함 여부, split child 간 겹침 여부처럼 PostGIS 공간 연산이 필요한 검증은 {@link
 * GeometrySpatialPort}에 위임한다.
 */
@Service
public class GeometryValidationService {
  ...
  public GeometryValidationService(
    ...
    this.geometryValidator =
        Objects.requireNonNull(geometryValidator, "도형 Validator는 null일 수 없습니다.");
    this.policy = Objects.requireNonNull(policy, "도형 정책은 null일 수 없습니다.");
    this.geometrySpatialPort =
        Objects.requireNonNull(geometrySpatialPort, "공간 검증 포트는 null일 수 없습니다.");
    this.geometryFactory = new GeometryFactory();
  }

  /**
   * overall_search_area 생성 또는 수정 요청의 Polygon을 검증한다.
   *
   * <p>공통 Polygon 검증을 수행한 뒤, 실제 면적이 최소 면적 기준 이상인지 확인한다.
   *
   * @param polygon GeoJSON Polygon 입력값
   * @return 도형 검증 결과
   */
  public GeometryValidationResult validateOverallSearchAreaPolygon(GeoJsonPolygon polygon) {
   ...
   *
   * <p>공통 Polygon 검증과 최소 면적 검증을 수행한 뒤, search_area가 active overall_search_area 내부에 완전히 포함되는지 확인한다.
   *
   * @param polygon GeoJSON Polygon 입력값
   * @param activeOverallAreaWkt active overall_search_area Polygon WKT
   * @return 검증과 정규화가 끝난 도형 결과
   */
  public GeometryValidationResult validateSearchAreaPolygon(
      GeoJsonPolygon polygon, String activeOverallAreaWkt) {
    if (activeOverallAreaWkt == null || activeOverallAreaWkt.isBlank()) {
      throw OverallSearchAreaRequiredException.overallSearchAreaRequired(
          "active overall_search_area geometry가 필요합니다.");
    }

    GeometryValidationResult result = validateCommonPolygon(polygon);

    validateMinimumArea(result);
    validateInsideActiveOverallArea(activeOverallAreaWkt, result);
   ...
   *
   * <p>각 child Polygon의 공통 검증과 최소 면적 검증을 수행한다.
   *
   * <p>이후 모든 child가 parent 내부에 포함되는지, child끼리 면적으로 겹치지 않는지 확인한다.
   *
  ...
  public List<GeometryValidationResult> validateSplitChildren(
      String parentAreaWkt, List<GeoJsonPolygon> childPolygons) {
    ...
    if (childPolygons == null || childPolygons.size() < 2) {
      throw InvalidGeometryException.invalidGeometry("split child Polygon은 최소 2개 이상이어야 합니다.");
    }

    List<GeometryValidationResult> results =
        childPolygons.stream().map(this::validateOverallSearchAreaPolygon).toList();
  ...
  /**
   * overall_search_area, search_area, split child에 공통으로 적용되는 Polygon 검증을 수행한다.
   *
   * <p>GeoJSON type과 outer ring을 확인하고, ring을 S2 기준으로 정규화한 뒤 JTS Polygon으로 변환한다.
   *
   * <p>JTS 기준의 빈 Polygon, 자기 교차, 0면적 여부까지 확인한다.
   *
   * @param polygon GeoJSON Polygon 입력값
   ...
   *
   * <p>빈 Polygon, 자기 교차, 0면적 Polygon을 거부한다.
   *
   * <p>실제 m² 기준 최소 면적 검증은 PostGIS 기반 {@link #validateMinimumArea(GeometryValidationResult)}에서 수행한다.
   *
  ...
  private void validateMinimumArea(GeometryValidationResult result) {
    boolean valid =
        geometrySpatialPort.isAreaAtLeastM2(result.wkt(), policy.minimumPolygonAreaM2());
  ...
  /**
   * search_area Polygon이 active overall_search_area 내부에 완전히 포함되는지 검증한다.
   *
   * <p>경계선에 닿는 경우까지 허용하기 위해 포함 여부는 PostGIS covers 검증에 위임한다.
   *
   * @param activeOverallAreaWkt active overall_search_area WKT
   * @param result search_area 검증 결과
   */
  private void validateInsideActiveOverallArea(
      String activeOverallAreaWkt, GeometryValidationResult result) {
    boolean covered = geometrySpatialPort.covers(activeOverallAreaWkt, result.wkt());

    if (!covered) {
      throw InvalidGeometryException.invalidGeometry(
          "search_area Polygon은 active overall_search_area 내부에 있어야 합니다.");
    }
  }
  ...
  private void validateAllChildrenInsideParent(
      String parentAreaWkt, List<GeometryValidationResult> children) {
    for (GeometryValidationResult child : children) {
      boolean covered = geometrySpatialPort.covers(parentAreaWkt, child.wkt());

      if (!covered) {
        throw InvalidGeometryException.invalidGeometry(
            "split child Polygon은 parent Polygon 내부에 있어야 합니다.");
      }
    }
  }
   ...
   *
   * <p>경계선만 맞닿는 경우는 겹침으로 보지 않고, 실제 면적이 겹치는 경우만 거부한다.
   *
   * @param children child Polygon 검증 결과 목록
   */
  private void validateChildrenDoNotOverlap(List<GeometryValidationResult> children) {
    for (int i = 0; i < children.size(); i++) {
      for (int j = i + 1; j < children.size(); j++) {
        boolean overlaps =
            geometrySpatialPort.overlapsByArea(children.get(i).wkt(), children.get(j).wkt());
