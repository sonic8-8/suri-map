 ...
 *
 * <p>외부 Firebase 호출 없이 메모리에 발송 기록을 남긴다.
 * 테스트에서 "누구에게, 어떤 payload로, 어떤 eventId로 보냈는지" 검증할 수 있다.</p>
 *
 * <p>하네스 시나리오(SC-02, SC-08)에서 mock FCM dispatcher 미수신을
 * 실패로 판정하는 failure injection도 지원한다.</p>
 */
public class MockFcmDispatcher implements FcmDispatcherPort {
    ...
    public DispatchResult send(List<String> recipients, Map<String, Object> payload, String eventId) {
        ...
        // 성공 기록 저장
        CapturedDispatch capture = new CapturedDispatch(
                eventId,
    ...
    public List<CapturedDispatch> findByRecipient(String recipient) {
        return dispatches.stream()
                .filter(d -> d.recipients().contains(recipient))
                .toList();
    }
    ...
    public List<CapturedDispatch> findByEventType(String eventType) {
        return dispatches.stream()
                .filter(d -> eventType.equals(d.payload().get("type")))
                .toList();
    }
