...
/** Insert shape for ReferenceMarkerSeed.createForIncident. */
public record MarkerSeedRecord(
    UUID incidentId,
    UUID id,
  ...
  public static MarkerSeedRecord from(UUID incidentId, SeedMarker seedMarker) {
    return new MarkerSeedRecord(
        incidentId,
        seedMarker.id(),
        seedMarker.operationalPeriodId(),
        seedMarker.dutyShiftId(),
        seedMarker.markerType(),
        seedMarker.supportRequestType(),
        seedMarker.location(),
        seedMarker.memo(),
        seedMarker.occurredAt(),
        seedMarker.createdByAccountId(),
        seedMarker.policePhoneId(),
