...
class SearchAreaReadRepositoryTest {

    @Test
    fun activeOverallUsesCanonicalSearchAreaQuery() = runBlocking {
        val callFactory = CapturingCallFactory(response = response(200, """{"status":"ACTIVE"}"""))
        val repository = SearchAreaReadRepository(
    ...
    @Test
    fun listUsesSearchAreaFiltersWithoutWriteHeaders() = runBlocking {
        val callFactory = CapturingCallFactory(response = response(200, """{"areas":[]}"""))
        val repository = SearchAreaReadRepository(
            apiClient = SuriMapApiClient(
