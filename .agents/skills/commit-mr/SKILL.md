---
name: commit-mr
description: Lane task ID(예 L1-T01)를 인자로 받아 사이클 마무리 묶음을 자동화한다. .agents/scratch/<task-id>.json 에서 사이클 메타를 읽고 git diff 기반으로 커밋 메시지를 작성한 뒤 사용자 confirm 후 commit + push + GitLab MR 생성을 수행한다. MR 본문은 .gitlab/merge_request_templates/default.md 양식을 영역에 맞게 채워 작성한다. MR 생성 성공 시 해당 scratch 파일을 삭제해 다음 사이클 사고를 막는다.
arguments: task-id
...
- `.agents/scratch/$task-id.json` 에서 메타 로드 (없으면 즉시 차단)
...
- 인자(task-id) 미입력
- `.agents/scratch/<task-id>.json` 이 없거나 비어 있음 (`/ticket-branch` 미실행 또는 이미 사이클 종료된 task)
...
- 사이클 메타는 `.agents/scratch/$task-id.json` 에서 로드한다.
...
1. `.agents/scratch/$task-id.json` 로드. 파일이 없거나 손상되면 `Failure / Ambiguity Format` 으로 중단 (사용자가 `/ticket-branch $task-id` 부터 다시 시작해야 함을 안내).
...
6. confirm 후 — `scripts/commit-push.sh commit` 호출 반복:
   ```bash
   .agents/skills/commit-mr/scripts/commit-push.sh commit \
     --file "<path1>" \
     --file "<path2>" \
     --message "$(printf '%s' '<커밋 메시지 본문>')"
   ```
   - 커밋 그룹별로 호출한다. 한 커밋에 여러 파일이 들어가면 `--file` 을 파일 수만큼 반복한다.
   - 메시지는 multi-line OK (heredoc 또는 따옴표로 보존).
   - 스크립트는 `.agents/scratch/`와 legacy `.claude/scratch/` 아래 파일 add 를 자체 거부한다.
7. push — `scripts/commit-push.sh push` 호출:
   ```bash
   .agents/skills/commit-mr/scripts/commit-push.sh push \
...
10. confirm 후 MR 생성 — `scripts/mr-create.sh` 호출.
    - 같은 source/target 의 열린 MR이 이미 있으면 `mr-create.sh` 는 새 MR을 만들지 않고 기존 MR URL 을 반환한다.
    - **`scratch.area_tag` → MR Label 매핑** (위 `MR Label 매핑` 표 적용):
      - 단일: `BE` → `⌨️ BE` 등
      - 결합(`BE/FE`) → 콤마 구분 결합 (`⌨️ BE,🖥️ FE`). 스크립트가 콤마 split 후 여러 `--label` 호출.
      - 매핑 실패 시 `glab label list -R s14-final/S14P31C106` 으로 fresh 조회 후 재매칭.
    ```bash
    MR_URL=$(.agents/skills/commit-mr/scripts/mr-create.sh \
...
`.gitlab/merge_request_templates/default.md` 의 양식을 따른다. 템플릿의 기본 섹션을 유지하되 아래 정보는 반드시 들어간다.

1. **변경 사항** — 표 형식 권장 (파일 / 정정 내용)
2. **관련 Jira 티켓** — `[<jira_key>](https://ssafy.atlassian.net/browse/<jira_key>)` 링크
3. **Task 추적** — task ID, Lane, task 파일, 담당 Spec, Phase
4. **기준 문서 변경** — 수정한 source-of-truth 문서 또는 `없음`
5. **검증 결과** — 실제로 실행한 명령과 결과
6. **Cross-lane impact** — 영향받는 Lane 또는 `없음`
7. **작성자 확인** — 실제로 한 검증 항목 (체크된 것)
8. **<영역> 리뷰 체크리스트** — area 에 맞춰 갈아끼움
...
- `.agents/scratch/<task-id>.json` 부재 또는 손상
...
- MR 생성 성공 시 `.agents/scratch/<task-id>.json` 을 삭제한다. 다음 사이클은 반드시 `/ticket-branch <task-id>` 부터 다시 시작하도록 강제한다.
- 중간 step 실패로 중단된 경우에는 scratch 를 보존한다 (재시도 가능).
- 다른 task 의 scratch 파일은 건드리지 않는다 (여러 사이클 동시 진행 가능).
- **`.agents/scratch/`와 legacy `.claude/scratch/` 아래 모든 파일은 절대 `git add` / `git commit` 대상에 포함하지 않는다. 사용자에게 커밋 제안조차 하지 않는다.** 변경 파일 목록을 사용자에게 보여줄 때도 scratch 파일은 제외해 표시한다.
