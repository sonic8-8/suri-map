...
1. 커밋 메시지는 `[Area] type(scope): 한글 요약 (<JIRA-KEY>)` 형식을 쓴다.
2. MR 제목은 커밋 제목과 같은 형식으로 둔다.
3. MR 설명에는 Jira key, 완료한 task, 검증 명령/결과, 영향 Lane, 수정한 기준 문서를 적는다.
4. GitLab labels, milestone, assignee, reviewer를 설정한다.
5. MR merge 후 Jira 상태와 TODO 체크박스를 실제 완료 기준에 맞게 갱신한다.
