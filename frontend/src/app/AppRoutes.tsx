...
import type { useIncidentWorkspaceState } from './useIncidentWorkspaceState';
import type { useMarkerNotificationQueue } from './useMarkerNotificationQueue';

type AppRoutesProps = {
  session: ReturnType<typeof useAppSession>;
  workspaceState: ReturnType<typeof useIncidentWorkspaceState>;
  markerNotificationQueue: ReturnType<typeof useMarkerNotificationQueue>;
};

export function AppRoutes({ session, workspaceState, markerNotificationQueue }: AppRoutesProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const { currentUserAccount, handleOidcCallbackFailure, handleOidcCallbackSuccess, openLogin } = session;
  const {
