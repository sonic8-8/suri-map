...
if ! find "$SOURCE_DIR" -mindepth 2 -maxdepth 2 -name N3A_B0010000.shp | grep -q .; then
  echo "N3A_B0010000.shp not found under $SOURCE_DIR" >&2
  exit 1
fi
  ...
  bash -lc '
    ...
    for shp in /source/*/N3A_B0010000.shp; do
      ...
      ogr2ogr \
        ...
        -t_srs EPSG:4326 \
        -dialect SQLite \
        -sql "SELECT ST_PointOnSurface(geometry) AS geometry, NAME AS name, UFID AS ufid, NMLY AS floor_count, '\''${district}'\'' AS district FROM N3A_B0010000 WHERE NAME IS NOT NULL AND TRIM(NAME) <> '\'''\''"
